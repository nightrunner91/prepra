# Prepra — подготовка к собеседованию Frontend-разработчика 2026
...
Vue 3 — прогрессивный фреймворк для построения интерфейсов. Раздел разбирает SFC, реактивность, Composition API, Vue Router, обработку ошибок, производительность, TypeScript и экосистему. Основной фокус — `<script setup>` и Composition API; Options API рассматривается как legacy для чтения чужого кода.

- [ ] Что такое Vue и чем он отличается от React и Angular (прогрессивный фреймворк vs библиотека vs монолит)
- [ ] Что такое SFC и из каких блоков состоит `.vue`-файл (`<script setup>`, `<template>`, `<style scoped>`)
- [ ] Чем Composition API отличается от Options API и почему новый код пишется на `<script setup>`
- [ ] Как работают `ref`, `reactive`, `computed`, `watch` и в чём между ними разница
- [ ] Как работают директивы `v-bind`, `v-on`, `v-model`, `v-if`, `v-for`, `v-show`
- [ ] Чем `v-if` отличается от `v-show` и когда что использовать
- [ ] Как передавать данные в компонент через `props` и почему их нельзя мутировать
- [ ] Как дочерний компонент сообщает родителю о событии через `defineEmits`/`emit`
- [ ] Что такое `v-model` и как он устроен под капотом (пропс + событие)
- [ ] Зачем нужен `:key` при `v-for` и почему индекс массива — плохой выбор
- [ ] Что такое слоты (`slot`) и какие они бывают (default, named, scoped)
- [ ] Как работает Vue Router: динамические маршруты, guards, lazy loading
- [ ] Что такое lifecycle hooks во Vue и с чем их сравнивать в React (`useEffect`/`useLayoutEffect`)
- [ ] Какие инструменты оптимизации производительности есть во Vue (`computed`, `v-memo`, `defineAsyncComponent`)
- [ ] Как типизировать пропсы, emits и composables с TypeScript

[Основы Vue 3](./docs/vue/vue-fundamentals.md) · [README раздела Vue](./docs/vue/README.md)

---

## 8. Nuxt

Nuxt 3 — full-stack фреймворк на базе Vue 3. Он добавляет файловую маршрутизацию, серверный рендеринг, Nitro, auto-imports composables, серверные роуты и унифицированный деплой. Раздел разбирает, как строить production-ready приложения на Nuxt, не теряя гибкости Vue.

- [ ] Что такое Nuxt и чем он отличается от Vue SPA на Vite (фреймворк vs сборщик)
- [ ] Как создать проект и из чего состоит стандартная структура папок
- [ ] Как работает файловая маршрутизация: `pages/`, динамические сегменты, вложенные роуты, `routeParam`, 404
- [ ] Какие режимы рендеринга поддерживает Nuxt (SSR, SSG, CSR, ISR) и где задаются `routeRules`
- [ ] Что такое Nitro, какие пресеты деплоя есть и почему один бандл работает везде
- [ ] Какие встроенные composables есть в Nuxt (`useFetch`, `useAsyncData`, `useState`, `useHead`, `useRoute`)
- [ ] Что такое auto-imports и как создавать собственные composables в `composables/`
- [ ] Как устроены плагины и middleware: порядок выполнения, глобальные и именованные middleware
- [ ] Как создавать серверные API-роуты в `server/api/` и чем они отличаются от `server/routes/`
- [ ] Как делать SSR-safe data fetching: `useFetch` vs `$fetch` vs `useAsyncData`
- [ ] Как управлять состоянием и SEO: `useState`, Pinia, `useHead`, `useSeoMeta`
- [ ] Как работает обработка ошибок: `error.vue`, `throwError`, `showError`
- [ ] Что такое Nuxt Layers и как они помогают масштабировать проекты
- [ ] Как задеплоить Nuxt 3: `nuxt build`, `nuxt generate`, Nitro presets, `routeRules`, runtime config

[README раздела Nuxt](./docs/nuxt/README.md)

---

## 9. Хранение данных и управление состоянием
...
## 11. Архитектура и паттерны проектирования
...
## 17. Платформы: React Native, PWA и Preact
