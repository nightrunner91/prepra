...
questions:
  - "Чем fetch отличается от axios при обработке HTTP-ошибок"
  - "Какие преимущества у ky перед axios"
  - "Зачем нужен централизованный API-клиент"
  - "Как настроить interceptors для добавления токена авторизации"
  - "Почему baseURL не должен хардкодиться"
...
В любом фронтенд-проекте рано или поздно появляется слой для общения с сервером. Голый `fetch` подходит для простых задач, но по мере роста проекта нужны базовый URL, interceptors, обработка ошибок и отмена запросов. Выбор HTTP-клиента влияет на архитектуру всего приложения.
...
- `fetch` — нативный API, не бросает исключение на HTTP-ошибках и не имеет interceptors.
- `axios` бросает исключение на `status >= 400`, поддерживает interceptors и timeout из коробки.
- `ky` — лёгкая обёртка над `fetch` (~4 KB) с автоматическим `.json()` и встроенным retry.
- `ofetch` заточен под SSR/CSR изоморфные запросы и используется в Nuxt 3.
- Централизованный API-клиент — единый экземпляр с baseURL, timeout, hooks и обработкой ошибок.
- Interceptors добавляют токен авторизации, request ID и логируют запросы в dev-режиме.
- Переменные окружения с публичными префиксами (`VITE_`, `NEXT_PUBLIC_`) попадают в клиентский бандл.
- Секретные ключи никогда не должны быть в клиентском бандле.
- Антипаттерны: дублирование baseURL, игнорирование HTTP-ошибок, разные клиенты в разных модулях.
