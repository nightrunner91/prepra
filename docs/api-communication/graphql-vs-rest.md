...
questions:
  ...
  - "Как устроена схема в GraphQL"
  - "Чем query отличается от mutation"
  - "Как работают subscriptions в GraphQL"
  - "Какие преимущества и недостатки GraphQL перед REST"
  - "Что такое BFF и зачем он нужен"
  - "Когда стоит выбрать REST, а когда GraphQL"
---

# GraphQL vs REST: когда что выбрать

При выборе способа взаимодействия клиента с сервером разработчик часто стоит между REST и GraphQL. Каждая технология решает свои задачи и имеет свои ограничения. Понимание их различий помогает выбрать правильный подход для конкретного проекта.

## Содержание

1. [Что такое REST](#что-такое-rest)
2. [Что такое GraphQL](#что-такое-graphql)
3. [Примеры кода](#примеры-кода)
4. [Сравнение GraphQL и REST](#сравнение-graphql-и-rest)
5. [Плюсы и минусы](#плюсы-и-минусы)
6. [Сложность для разработчиков](#сложность-для-разработчиков)
7. [Когда что использовать](#когда-что-использовать)
8. [Компромиссные решения](#компромиссные-решения)
...
- REST — архитектурный стиль с ресурсами, HTTP-методами и stateless-запросами.
- GraphQL — язык запросов с единым endpoint, клиент сам определяет структуру ответа.
- Over-fetching — получение лишних данных, under-fetching — необходимость нескольких запросов. GraphQL решает обе проблемы.
- Схема GraphQL определяет типы данных, query, mutation и subscription.
- Резолверы — функции, которые загружают данные для каждого поля схемы.
- Apollo Client предоставляет хуки useQuery, useMutation, useSubscription и useLazyQuery.
- GraphQL требует решения проблемы N+1 через DataLoader.
- Кэширование в GraphQL сложнее, чем в REST, где работает HTTP-кэш.
- REST подходит для простых CRUD API, GraphQL — для сложных UI с вложенными данными.
- BFF — паттерн отдельного слоя агрегации данных для каждого клиента.
- Компромисс: REST для простых операций, GraphQL для сложных запросов.
