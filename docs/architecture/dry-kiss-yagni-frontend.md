...
questions:
  - "Что означают DRY, KISS и YAGNI и в чём разница между ними"
  - "Как в React выделять общую логику в хук, соблюдая DRY"
  - "Как в Vue реализовать DRY через composables"
  - "Почему использование Redux для локального состояния формы нарушает KISS"
  - "Какие признаки нарушения принципа YAGNI"
  - "Когда сознательно нарушать DRY и оставлять дублирование"
  - "Что такое preliminary abstraction и почему это антипаттерн"
  - "Как отличить необходимую абстракцию от искусственной"
---
...
- DRY борется с дублированием: общая логика выносится в один источник истины — хук в React, composable во Vue.
- KISS требует начинать с простого: Redux для формы, фабрика для двух кнопок или render-функция для инпута — переусложнение.
- YAGNI запрещает писать код «на всякий случай»: 5 типов авторизации, когда используется JWT — мусор в кодовой базе.
- DRY, KISS и YAGNI дополняют друг друга: избыточная абстракция ради DRY часто нарушает KISS.
- Дублирование дешевле неверной абстракции: 3 похожих куска кода лучше одной сложной обобщённой функции с 15 параметрами.
- В React принципы проявляются через custom hooks, локальное состояние и отсутствие лишних HOC/абстракций.
- Во Vue — через composables, `ref`/`reactive` вместо Vuex/Pinia для локального состояния и минимальные пропсы.
- Признак нарушения DRY — исправление одного бага требует правок в нескольких файлах.
- Признак нарушения KISS — код сложнее самой задачи и требует комментариев для чтения.
- Признак нарушения YAGNI — «мёртвые ветки» и опции, которые никогда не используются.
- Отступать от принципов оправдано в трёх случаях: абстракция усложняет код, задача действительно сложная, есть подтверждённые будущие требования.
