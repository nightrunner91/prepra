...
questions:
  - "Как реализовать паттерн Абстрактная фабрика в React через Context"
  - "Как реализовать Абстрактную фабрику во Vue через provide/inject"
  - "Чем HOC-декораторы отличаются от хуков-декораторов"
  - "Почему классический Singleton считается антипаттерном во frontend"
  - "Как ES-модули реализуют паттерн Singleton по умолчанию"
  - "Почему синглтоны опасны в SSR-окружении"
  - "Как встроенная реактивность React и Vue реализует паттерн Observer"
  - "Что делает `useSyncExternalStore` и когда его использовать"
  - "Как EventBus реализует Observer для межкомпонентного общения"
---
...
- Классические паттерны GoF во frontend не исчезли — они реализуются идиомами React и Vue.
- Абстрактная фабрика в React живёт через Context + фабричные объекты (темизация, платформы, A/B тесты).
- Во Vue та же роль — у `provide/inject` в связке с динамическими компонентами `<component :is>`.
- Декоратор реализуется в React через HOC, JSX-обёртки и, в современном виде, через custom hooks.
- Во Vue декораторами становятся директивы (`v-tooltip`, `v-focus`), composables и обёрточные компоненты.
- Синглтон в чистом виде считается антипаттерном: сложно тестировать и опасен в SSR из-за общего состояния между запросами.
- ES-модули по своей природе — синглтоны: один импорт возвращает один и тот же экземпляр.
- Реактивность React (`useState`, Context) и Vue (`ref`, `reactive`, `watch`) — это встроенный Observer.
- `useSyncExternalStore` (React 18+) — специальный хук для интеграции с внешними Observer-сторами.
- EventBus — простой способ реализовать Observer для несвязанных компонентов, но при большом числе подписчиков усложняет отладку.
- Паттерны — это про решения повторяющихся проблем, а не про синтаксис классов.
