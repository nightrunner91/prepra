...
questions:
  - "Как Абстрактная фабрика трансформировалась из классового ООП в идиомы React (Context) и Vue (provide/inject)"
  - "Чем HOC-декораторы отличаются от хуков-декораторов и в каких случаях какой подход предпочтительнее"
  - "Почему классический Singleton считается антипаттерном во frontend и как ES-модули реализуют его безопасно"
  - "Почему синглтоны опасны в SSR-окружении и как это влияет на архитектуру state management"
  - "Как встроенная реактивность React и Vue реализует паттерн Observer и когда нужен `useSyncExternalStore`"
  - "Как EventBus реализует Observer для межкомпонентного общения и какие у этого подхода ограничения"
---

# Паттерны GoF в React и Vue

Классические паттерны Gang of Four появились в 1994 году в мире ООП с наследованием и интерфейсами. Во frontend классов почти нет — но проблемы, которые решали паттерны, никуда не делись. В статье разберём, как четыре ключевых паттерна GoF — Абстрактная фабрика, Декоратор, Синглтон и Наблюдатель — трансформировались в идиомы React (Context, хуки, HOC) и Vue (composables, provide/inject, реактивность).
...
- Классические паттерны GoF не исчезли во frontend — они трансформировались под идиомы React и Vue: паттерны — это про решения повторяющихся проблем, а не про синтаксис классов.
- Абстрактная фабрика в React реализуется через Context + фабричные объекты (темизация, платформы, A/B тесты), во Vue — через `provide/inject` в связке с динамическими компонентами `<component :is>`.
- Декоратор в React реализуется через HOC, JSX-обёртки и custom hooks; во Vue — через директивы (`v-tooltip`, `v-focus`), composables и обёрточные компоненты.
- Синглтон в чистом виде — антипаттерн: сложно тестировать и опасно в SSR из-за общего состояния между запросами; ES-модули по природе являются синглтонами (один импорт — один экземпляр).
- Встроенная реактивность React (`useState`, Context) и Vue (`ref`, `reactive`, `watch`) — это реализация Observer; `useSyncExternalStore` (React 18+) позволяет интегрироваться с внешними Observer-сторами.
- EventBus — простой Observer для несвязанных компонентов, но при большом числе подписчиков усложняет отладку; предпочтительнее Context/provide-inject для явных зависимостей.
