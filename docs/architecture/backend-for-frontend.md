...
questions:
  ...
  - "Чем BFF отличается от API Gateway и почему их часто используют вместе"
  - "Как Server Components в Next.js реализуют BFF-паттерн и почему их называют встроенным BFF"
  - "Как BFF выступает в роли Anti-Corruption Layer между backend-сервисами и фронтендом"
  - "Кто должен владеть BFF: фронтенд-команда или бэкенд, и как разделить ответственность"
  - "Почему в SSR-окружении опасно хранить пользовательское состояние в модуле-синглтоне BFF"
  - "В каких случаях сложность BFF не окупается и от него стоит отказаться"
---

# Backend for Frontend (BFF)

BFF — архитектурный паттерн, при котором для каждого клиента (web, mobile, IoT) создаётся отдельный backend-сервис, оптимизированный под его нужды. Вместо одного универсального API каждый клиент получает свой слой, который агрегирует данные из микросервисов и трансформирует их под формат, удобный конкретному UI. В статье разберём зачем нужен BFF, как реализовать его в Next.js и какие проблемы он снимает с фронтенда.

## Содержание

1. [Что такое BFF](#что-такое-bff)
2. [Зачем нужен BFF](#зачем-нужен-bff)
3. [Архитектура BFF](#архитектура-bff)
4. [Преимущества BFF](#преимущества-bff)
5. [Когда использовать BFF](#когда-использовать-bff)
6. [Пример реализации](#пример-реализации)
7. [Технологии для BFF](#технологии-для-bff)
8. [Best Practices](#best-practices)
9. [BFF vs API Gateway](#bff-vs-api-gateway)
10. [BFF в Next.js](#bff-в-nextjs)
11. [Кто должен писать и поддерживать BFF](#кто-должен-писать-и-поддерживать-bff)
12. [Решает ли BFF проблему несоответствия формата данных](#решает-ли-bff-проблему-несоответствия-формата-данных)
...
- BFF — отдельный backend-сервис под каждый клиент (web, mobile, IoT), который агрегирует данные из микросервисов и трансформирует их под формат конкретного UI, решая over-fetching, under-fetching и client-specific оптимизацию.
- В Next.js BFF реализуется через Route Handlers, Server Actions и Server Components — последние фактически являются встроенным BFF: выполняются на сервере и отдают уже собранные данные.
- BFF ≠ API Gateway: Gateway — единая точка входа с routing и auth для всех клиентов, BFF — client-specific агрегация и трансформация; часто они используются вместе.
- BFF выступает как Anti-Corruption Layer: защищает domain-модель фронтенда от чужеродных форматов и контрактов backend-сервисов.
- Оптимальная модель ответственности — совместная: фронтенд определяет контракт данных, бэкенд отвечает за инфраструктуру и безопасность.
- В SSR-окружении опасно хранить пользовательское состояние в модуле-синглтоне — оно «протекает» между запросами разных пользователей.
- BFF не оправдан для простых приложений с одним клиентом или маленьких команд — он добавляет инфраструктурную сложность, которая окупается только при нескольких клиентах с разными требованиями.
...
Ключевые моменты:
1. Создавайте BFF, когда у вас несколько клиентов с разными требованиями.
2. Используйте BFF для агрегации данных из микросервисов.
3. Оптимизируйте ответы под специфику каждого клиента.
4. Не забывайте про мониторинг и кэширование.
5. Рассмотрите GraphQL как дополнение или альтернативу.
...
- [Anti-Corruption Layer — Microsoft Learn](https://learn.microsoft.com/en-us/azure/architecture/patterns/anti-corruption-layer)
