...
questions:
  - "Что такое BFF и какую проблему он решает"
  - "Чем BFF отличается от API Gateway"
  - "Почему универсальный API плохо подходит для мобильного и web-клиента одновременно"
  - "Как Server Components в Next.js реализуют BFF-паттерн"
  - "Кто должен писать и поддерживать BFF: фронтенд или бэкенд команда"
  - "Как BFF решает проблему несоответствия формата данных backend и UI"
  - "Что такое Anti-Corruption Layer и как BFF выступает в этой роли"
  - "В каких случаях BFF не оправдан и его не стоит вводить"
  - "Какие риски у синглтон-BFF в SSR-окружении"
---
...
- BFF — отдельный backend под каждый клиент (web, mobile, IoT), а не универсальный API для всех.
- Решает проблемы over-fetching, under-fetching и client-specific оптимизации.
- Агрегирует данные из микросервисов и трансформирует их под формат конкретного UI.
- В Next.js BFF реализуется через Route Handlers, Server Components и Server Actions.
- Server Components — фактически встроенный BFF: выполняются на сервере, отдают уже собранные данные.
- BFF ≠ API Gateway: Gateway — единая точка входа с routing/auth, BFF — client-specific агрегация. Часто используются вместе.
- Выступает как Anti-Corruption Layer: защищает domain-модель фронтенда от чужеродных форматов backend.
...
- В SSR-окружении опасно хранить пользовательское состояние в модуле-синглтоне — оно «протекает» между запросами.
- Не оправдан для простых приложений с одним клиентом или маленьких команд.
