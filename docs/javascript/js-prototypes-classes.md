...
- Каждый объект имеет скрытое свойство `[[Prototype]]`, ссылающееся на другой объект; поиск свойства идёт по цепочке прототипов до `null`.
- `Object.create(proto)` создаёт новый объект с заданным прототипом, а `Object.setPrototypeOf` меняет прототип существующего объекта — это медленная операция.
- Наследование без `class` реализуется через функции-конструкторы, связывание прототипов через `Object.create` и вызов родительского конструктора через `.call`.
- При `new Constructor()` создаётся новый объект, его `[[Prototype]]` связывается с `Constructor.prototype`, вызывается конструктор, и объект возвращается.
- `super()` в производных классах вызывает конструктор родителя и обязателен до первого обращения к `this`.
- Приватные поля и методы объявляются через `#` и доступны только внутри класса, где объявлены; они не наследуются.
- Статические методы принадлежат самому классу, а не экземплярам; они используются для фабрик и утилит и наследуются подклассами.
- `instanceof` проверяет наличие конструктора в цепочке прототипов, но может обмануть при изменённом прототипе или объектах из других iframe.
- Классы в JavaScript — это синтаксический сахар над прототипным наследованием, в отличие от классового наследования в Java/C++.

## Заключение

JavaScript использует прототипное наследование: каждый объект делегирует поиск свойств своему прототипу. Синтаксис `class` из ES6 упрощает работу, но под капотом остаются те же прототипы, конструкторы и цепочки. Понимание этих механизмов необходимо для написания корректного кода, прохождения собеседований и эффективного использования наследования.
