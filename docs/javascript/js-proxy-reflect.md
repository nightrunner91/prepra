...
- `Proxy` оборачивает объект и перехватывает операции через ловушки (traps), позволяя контролировать чтение, запись, удаление, вызов функций и создание объектов через `new`.
- Основные ловушки `Proxy`: `get`, `set`, `has`, `deleteProperty`, `ownKeys`, `apply`, `construct`, `getPrototypeOf`/`setPrototypeOf`, `defineProperty`, `getOwnPropertyDescriptor`.
- Ловушка `get` используется для значений по умолчанию и логирования чтения, а `set` — для валидации записываемых значений.
- `Reflect` дублирует внутренние операции движка и используется в ловушках Proxy для делегирования стандартного поведения оригинальному объекту.
- `Reflect` отличается от прямого доступа тем, что корректно передаёт `receiver`, возвращает булевы результаты и соответствует ловушкам Proxy один к одному.
- Параметр `receiver` в `Reflect.get`/`Reflect.set` обеспечивает корректный `this` при работе с прототипами и getter/setter.
- Vue 3 строит реактивность на `Proxy`, оборачивая объекты в `reactive()` и используя `track`/`trigger` для сбора зависимостей и запуска эффектов.
- Реактивность Vue 3 на `Proxy` отличается от Vue 2 на `Object.defineProperty` тем, что отслеживает добавление и удаление свойств, изменение массивов по индексу и не требует `Vue.set`.
- Ограничения `Proxy`: нельзя создать над примитивом, `typeof` не отличает прокси от объекта, некоторые операции (`===`, `instanceof`) не перехватываются, производительность ниже прямого доступа.
- `Proxy.revocable` создаёт прокси, который можно отключить вызовом `revoke()`, после чего любая операция выбрасывает `TypeError`.

## Заключение

`Proxy` и `Reflect` — мощный инструмент метапрограммирования в JavaScript. `Proxy` позволяет перехватывать любые операции над объектами, а `Reflect` обеспечивает корректное делегирование стандартного поведения в ловушках. Именно эта пара лежит в основе реактивности Vue 3, заменившей `Object.defineProperty` из Vue 2. При использовании `Proxy` важно помнить о его ограничениях: невозможности работы с примитивами, накладных расходах на производительность и непрозрачности для некоторых операторов. Потробуйтесь реализовать простую реактивную обёртку или валидатор — это закрепит понимание ловушек и `receiver`.

## Полезные ссылки

- [Proxy — MDN](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Proxy)
- [Reflect — MDN](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Reflect)
- [JavaScript Proxy — javascript.info](https://javascript.info/proxy)
- [Reactivity in Depth — Vue 3](https://vuejs.org/guide/extras/reactivity-in-depth.html)
