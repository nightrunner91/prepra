...
- `Promise.all` ожидает все промисы и отклоняется при первом отказе; `Promise.race` возвращает результат первого завершившегося промиса.
- `Promise.allSettled` нужен, когда важно получить результат каждого промиса независимо от успеха или отказа.
- `async/await` — синтаксический сахар над Promise; код после `await` выполняется как микрозадача.
- Ошибки в `async/await` обрабатываются через `try/catch`, в цепочках Promise — через `.catch()`.
- Необработанный отказ промиса вызывает событие `unhandledrejection`; его можно отловить глобально.
- `AbortController` позволяет отменить `fetch` и другие асинхронные операции, передавая `signal` в запрос.
- Независимые запросы лучше выполнять параллельно через `Promise.all`, зависимые — последовательно через `await`.
- `Promise.then` выполняется раньше `setTimeout(..., 0)`, потому что микрозадачи имеют приоритет над макрозадачами в Event Loop.

## Заключение

Promise и `async/await` — основа асинхронного программирования в JavaScript. Понимание состояний промиса, приоритета микрозадач над макрозадачами и методов комбинирования (`all`, `race`, `allSettled`, `any`) позволяет писать эффективный и предсказуемый код. Обработка ошибок через `try/catch` или `.catch()` и отмена операций через `AbortController` — обязательные навыки для стабильной работы с асинхронностью. Потренируйтесь переписывать вложенные callback'и на `async/await` и выбирать между последовательным и параллельным выполнением запросов.
