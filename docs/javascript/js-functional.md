...
- Чистая функция всегда возвращает одинаковый результат для одинаковых аргументов и не имеет побочных эффектов.
- Иммутабельность важна, потому что React использует сравнение по ссылке для оптимизации рендеринга (`React.memo`, `useEffect`).
- `reduce` сворачивает массив к одному значению и подходит для сумм, группировки, трансформации в объекты.
- `map` возвращает новый массив той же длины, а `forEach` ничего не возвращает и мутирует через побочные эффекты.
- Каррирование превращает `f(a, b)` в `f(a)(b)`, частичное применение фиксирует часть аргументов через `bind` или замыкание.
- `compose` выполняет функции справа налево, `pipe` — слева направо; результат одинаковый, направление чтения разное.
- `debounce` откладывает вызов до паузы между событиями, `throttle` гарантирует регулярный вызов не чаще заданного интервала.
- Иммутабельное обновление объекта: `{ ...obj, key: newValue }`, массива: `[...arr, newItem]` или `arr.map(...)`.

## Заключение

Функциональное программирование в JavaScript — это набор практик, которые делают код предсказуемым и удобным для тестирования. Чистые функции без побочных эффектов, иммутабельные данные, методы массивов `map`/`filter`/`reduce` и композиция функций — основные инструменты ФП в повседневной разработке. `debounce` и `throttle` решают проблему частых вызовов при обработке событий. Применяйте эти приёмы осознанно: не нужно переписывать весь код в функциональном стиле, но ключевые идеи значительно улучшают качество кода.

## Полезные ссылки

- [Array.prototype methods — MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array#instance_methods)
- [Functional Programming in JavaScript — MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Fundamentals#functional_programming)
- [Immer — Immutable State in JavaScript](https://immerjs.github.io/immer/)
