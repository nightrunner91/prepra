...
title: "Замыкания"
section: javascript
description: "Замыкание — это функция, которая «помнит» переменные из лексического окружения, в котором была создана, даже когда выполняется в другом месте. Это одна из самых важных концепций JavaScript: она объ..."
order: 2
tags: ["closures", "lexical-environment", "currying", "private-variables", "stale-closure"]
questions:
...
- Замыкание — это функция вместе со ссылкой на внешнее лексическое окружение, в котором она была создана.
- Внутренняя функция сохраняет доступ к переменным внешней функции, потому что хранит ссылку на её лексическое окружение.
- Приватные переменные создаются через замыкание: данные доступны только методам, возвращённым из внешней функции.
- Каррирование отличается от обычной функции тем, что каждая промежуточная функция замыкает ранее переданный аргумент.
- Проблема `setTimeout` в цикле с `var` решается заменой `var` на `let` или использованием IIFE для создания отдельного замыкания на каждой итерации.
- Модули ES — это замыкания: переменные верхнего уровня модуля скрыты и доступны только через экспортированный интерфейс.
- Каждый хук React замыкает переменные текущего рендера; массив зависимостей `useEffect` управляет актуальностью этих замыканий.
- Stale closure возникает, когда функция использует устаревшее значение из замыкания; исправляется добавлением зависимостей или функциональным обновлением состояния.

## Заключение

Замыкания — одна из ключевых концепций JavaScript. Функция сохраняет доступ к переменным внешнего лексического окружения даже после завершения внешней функции. Это объясняет работу приватных переменных, каррирования, модулей и React-хуков. Понимание замыканий необходимо для избежания stale closures и написания предсказуемого кода. Разберите классические задачи с `var` в цикле и фабрики функций, чтобы закрепить тему.

## Полезные ссылки

- [MDN: Closures](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Closures)
- [ECMAScript Specification: Lexical Environments](https://tc39.es/ecma262/#sec-lexical-environments)
- [React: Hooks and State](https://react.dev/learn/state-a-components-memory)
