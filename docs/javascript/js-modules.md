...
- ES Modules (ESM) — современный стандарт с `import`/`export`, CommonJS (CJS) — классическая система Node.js с `require`/`module.exports`.
- Именованные экспорты позволяют экспортировать несколько значений, дефолтный экспорт — один на модуль.
- В CJS `exports` — это ссылка на `module.exports`; перезапись `exports` ломает экспорт.
- Динамический импорт `import()` возвращает Promise и позволяет загружать модули по условию или лениво.
- Циклические зависимости возникают при взаимном импорте модулей; их устраняют выносом общего кода или динамическим импортом.
- В ESM работают live bindings — импортированные переменные синхронизируются с экспортом, в CJS значения копируются на момент `require`.
- Tree shaking удаляет неиспользуемый код при сборке и эффективнее работает со статическими ESM-импортами.
- ESM предпочтителен для новых проектов благодаря статическому анализу и эффективному tree shaking; CJS остаётся для поддержки существующего кода в Node.js.

## Заключение

Модули — основа структуры любого JavaScript-приложения. ESM и CJS решают одну задачу, но делают это по-разному: ESM опирается на статический анализ и live bindings, а CJS — на динамическое выполнение и копирование значений. Понимание их различий помогает правильно организовать код, избежать циклических зависимостей и эффективно использовать tree shaking.

## Полезные ссылки

- [Modules — MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules)
- [import — MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/import)
- [CommonJS vs ES Modules](https://nodejs.org/api/esm.html#commonjs-vs-es-modules)
