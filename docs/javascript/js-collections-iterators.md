...
order: 3
tags: ["коллекции", "итераторы", "генераторы"]
questions:
...
- `Map` принимает любые типы ключей и сохраняет порядок вставки, в отличие от `Object`, который приводит ключи к строкам.
- `Map` предпочтительнее объекта, когда ключи — не строки, важен порядок или нужен частый перебор.
- `Set` обеспечивает уникальность через сравнение `SameValueZero`: дубликаты игнорируются при добавлении.
- `WeakMap` и `WeakSet` хранят слабые ссылки на объекты и не препятствуют сборке мусора, что предотвращает утечки памяти.
- Итератор — объект с методом `next()`, возвращающим `{ value, done }`; итерируемый объект реализует `[Symbol.iterator]`.
- `Symbol.iterator` — встроенный символ, метод которого возвращает итератор; его реализуют массивы, строки, `Map`, `Set`.
- `for...of`, spread-оператор и деструктуризация работают через протокол итераторов, вызывая `[Symbol.iterator]()` автоматически.
- Генератор (`function*`) — функция, приостанавливающая выполнение через `yield` и возвращающая итератор.
- В генератор можно передавать значения через `next(value)`, что позволяет реализовать двустороннюю связь.

## Заключение

`Map`, `Set`, `WeakMap` и `WeakSet` расширяют стандартные возможности хранения данных в JavaScript. Протокол итераторов через `Symbol.iterator` объединяет массивы, строки, коллекции и пользовательские объекты, позволяя использовать `for...of`, spread и деструктуризацию единообразно. Генераторы упрощают создание итераторов и поддерживают ленивые вычисления. Понимание этих тем необходимо для эффективной работы с данными и прохождения собеседований.
...
- [ECMAScript Specification: Iterators and Generators](https://tc39.es/ecma262/#sec-iterable-interface)
