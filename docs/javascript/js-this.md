...
- Значение `this` определяется контекстом вызова функции, а не местом её объявления.
- В глобальном контексте `this` указывает на глобальный объект, при обычном вызове функции в строгом режиме — `undefined`.
- При вызове функции как метода объекта `this` указывает на объект, стоящий перед точкой.
- Конструктор с `new` создаёт новый объект и привязывает `this` к нему; вызов функции-конструктора без `new` приводит к утечке `this` в глобальную область.
- `call` вызывает функцию с явно указанным `this` и аргументами через запятую, `apply` — с аргументами массивом, `bind` — возвращает новую функцию с навсегда привязанным `this`.
- Потеря `this` происходит при передаче метода как callback — объект теряется, и `this` становится `undefined`; исправляется обёрткой в стрелочную функцию, `bind` или сохранением `this` в переменную.
- Стрелочные функции не имеют собственного `this` и захватывают его из внешнего лексического окружения в момент создания.
- В методах класса `this` указывает на экземпляр, но при передаче метода как callback контекст теряется — это исправляется привязкой в конструкторе или стрелочными полями класса.
- В классовых компонентах React методы нужно привязывать, потому что React вызывает обработчики как обычные функции, где `this` будет `undefined` в строгом режиме.
- Стрелочная функция внутри метода сохраняет правильный `this`, потому что захватывает его из окружающего метода в момент определения.

## Заключение

`this` — одна из ключевых концепций JavaScript, значение которой определяется способом вызова функции, а не местом её определения. Стрелочные функции решают большинство проблем с потерей контекста, потому что захватывают `this` из внешнего окружения. В классах и React-компонентах важно явно привязывать методы через `bind` в конструкторе или использовать стрелочные поля класса. Понимание `this`, `call`/`apply`/`bind` и разницы между обычной и стрелочной функциями — обязательный навык для уверенной работы с JavaScript.

## Полезные ссылки

- [this — MDN](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Operators/this)
- [Function.prototype.bind() — MDN](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Function/bind)
- [Стрелочные функции — MDN](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Functions/Arrow_functions)
- [Контекст выполнения — javascript.info](https://javascript.info/object-methods)
