...
section: html-css
description: "Flexbox — первый по-настоящему осевой layout-механизм в CSS. Он решает задачи, которые раньше требовали float, inline-block и кучу хаков: выравнивание по центру, равномерное распределение места, ад..."
order: 3
...
Flexbox — модель одномерной раскладки, в которой главную роль играет ось. Понимание алгоритма распределения пространства через `flex-grow`, `flex-shrink` и `flex-basis` позволяет точно управлять размерами без магических значений. Для равных колонок используйте `flex: 1` (basis 0%), для пропорциональных по контенту — `flex: auto`. Не забывайте про `min-width: 0` при обрезке текста и про то, что `order` не меняет порядок в DOM — это критично для доступности.
