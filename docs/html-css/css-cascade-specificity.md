...
questions:
  - "Какой алгоритм использует браузер для разрешения конфликтов между CSS-декларациями?"
  - "Что такое origin в CSS-каскаде и какие три источника стилей существуют?"
  - "Как `!important` меняет порядок приоритетов, и почему author `!important` уступает user `!important`?"
  - "Как `@layer` управляет приоритетом каскада без изменения специфичности?"
  - "Как рассчитывается специфичность селектора (A, B, C)?"
  - "Какой вес добавляют `:where()`, `:is()` и `:not()` к специфичности?"
  - "Какие свойства наследуются по умолчанию в CSS?"
  - "Чем отличаются `initial`, `unset` и `revert`?"
  - "Что происходит с правилами вне `@layer` — как они соотносятся с правилами внутри слоёв?"
  - "Почему `all: unset` на кнопке может сломать доступность?"
---
