...
questions:
  - "В каком порядке браузер разрешает конфликты между CSS-декларациями: origin, layer, specificity, source order"
  - "Как `!important` меняет порядок приоритетов и почему author `!important` уступает user `!important`"
  - "Как `@layer` управляет приоритетом без изменения специфичности и что происходит с правилами вне слоёв"
  - "Как рассчитывается специфичность (A, B, C) и как `:where()`, `:is()`, `:not()` влияют на её вес"
  - "Какие свойства наследуются по умолчанию и чем отличаются `inherit`, `initial`, `unset`, `revert`"
  - "Почему `all: unset` на интерактивном элементе может сломать доступность"
---
...
- Cascade разрешает конфликты в порядке: origin/importance → layer → specificity (A, B, C) → порядок появления. Origin-уровни: user agent < user < author; `!important` инвертирует порядок внутри своего уровня.
- `@layer` управляет приоритетом независимо от specificity: позже объявленный слой побеждает; правила вне `@layer` имеют приоритет выше любых слоёв.
- Specificity — это `(A, B, C)`: ID → классы/атрибуты/псевдоклассы → типы/псевдоэлементы. `:where()` обнуляет вклад, `:is()` и `:not()` берут максимальный вес аргумента.
- Наследование передаёт типографику, цвет и подобные свойства потомкам; ключевые слова `inherit`, `initial`, `unset`, `revert` дают точный контроль без переписывания стилей.
- `!important` — инструмент для утилитарных классов и accessibility-переопределений, а не для повседневной борьбы со специфичностью.

## Заключение

Cascade — это предсказуемая иерархия приоритетов: origin и importance, затем layer, специфичность и порядок появления. Понимание этой системы позволяет избегать борьбы со специфичностью через `!important` или искусственно длинные селекторы. `@layer` — современный инструмент для управления приоритетами в больших проектах, особенно при подключении сторонних библиотек. Наследование и ключевые слова `inherit`, `initial`, `unset`, `revert` дают точный контроль над значениями свойств без переписывания всего стека стилей.
