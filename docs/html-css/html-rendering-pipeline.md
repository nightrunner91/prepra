...
questions:
  - "Каковы этапы превращения HTML в пиксели на экране?"
  - "Что такое preload scanner и какую задачу он решает?"
  - "Почему браузер должен получить весь CSS перед построением Render Tree?"
  - "Какие свойства вызывают layout, какие — paint, какие — только composite?"
  - "Что такое forced synchronous layout и как его избежать?"
  ...
  - "Почему чтение `offsetWidth` после изменения стиля вызывает принудительный layout?"
  - "Зачем встраивать критический CSS inline?"
  - "Как браузер определяет, что элемент нужно продвинуть в отдельный compositor layer?"
---
