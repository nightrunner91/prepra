...
Адаптивный CSS эволюционировал от «ширина экрана определяет всё» к компонентно-ориентированной модели. Container queries позволяют компоненту реагировать на своё доступное пространство, а не на размер экрана. Viewport units нового поколения (`dvh`, `svh`) решают давнюю проблему с мобильными браузерами. `prefers-*`-запросы позволяют уважать системные настройки пользователя: тему, доступность и анимации. Современная практика — использовать media queries для глобального макета страницы и container queries для адаптации компонентов.
