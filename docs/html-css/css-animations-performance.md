---
title: "Transitions, animations и производительность CSS"
section: html-css
description: "CSS-анимации — не просто «украшательство». На собеседованиях их спрашивают через призму производительности: почему одни свойства тормозят, а другие нет, когда стоит использовать `will-change`, что ..."
order: 1
...
Производительность CSS-анимаций определяется тем, какие этапы рендеринга они затрагивают. `transform` и `opacity` работают на compositor thread и не требуют layout или paint — это основа плавных анимаций. `will-change` помогает браузеру заранее подготовить слой, но требует аккуратного использования: каждый слой расходует память. `contain` изолирует область перерисовки, а `content-visibility: auto` откладывает рендеринг элементов вне viewport. Финальное правило: уважайте настройку `prefers-reduced-motion` — анимации должны помогать, а не создавать дискомфорт.
