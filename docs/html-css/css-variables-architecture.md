...
questions:
  - "Чем CSS custom properties отличаются от переменных препроцессоров (Sass, Less)?"
  - "Как работает наследование custom properties по DOM?"
  - "Когда резервное значение `var(--name, fallback)` НЕ сработает?"
  - "Почему для вычислений с custom properties нужен `calc()`?"
  - "Какие три уровня токенов принято выделять в дизайн-системах?"
  - "Как `@layer` помогает управлять каскадом в большой CSS-архитектуре?"
  - "В чём суть методологии BEM и каковы её ограничения?"
  - "Чем CUBE CSS отличается от классического BEM?"
  - "Как CSS Modules обеспечивают инкапсуляцию классов?"
  - "Как совместить CSS Modules и CSS custom properties для темизации?"
---
