---
title: "CSS custom properties, архитектура стилей и подходы к инкапсуляции"
section: html-css
description: "CSS на больших проектах перестаёт быть просто набором правил: появляется необходимость в системе токенов, темизации, масштабируемой методологии именования и способе изолировать стили компонентов. C..."
order: 10
tags: ["custom-properties", "design-tokens", "bem", "itcss", "layer"]
questions:
  - "Чем CSS custom properties отличаются от переменных препроцессоров и как работает их наследование по DOM"
  - "Когда резервное значение `var(--name, fallback)` не сработает и почему для вычислений нужен `calc()`"
  - "Какие три уровня токенов (primitive, semantic, component) выделяют в дизайн-системах и зачем"
  - "Как `@layer` помогает управлять каскадом в большой CSS-архитектуре без повышения специфичности"
  - "В чём суть методологий BEM, CUBE и ITCSS и как они решают проблему масштабируемости стилей"
  - "Как CSS Modules обеспечивают инкапсуляцию классов и как совместить их с custom properties для темизации"
---
...
- Custom properties живут в runtime браузера, наследуются по DOM и поддерживают области видимости через селекторы — в отличие от переменных препроцессоров, которые компилируются в статические значения.
- `var(--name, fallback)` задаёт резервное значение, но оно не сработает, если переменная объявлена с невалидным для свойства значением; для вычислений используется `calc()`, так как сами переменные — строки.
- Токены делят на три уровня: примитивные (цвета, размеры, шрифты), семантические (ролевые: primary, danger, text), компонентные (специфичные для компонента: button-bg, card-border).
- Методологии именования: BEM — Block__Element--Modifier; CUBE — Composition, Utility, Block, Exception; ITCSS — иерархия от глобального к локальному (settings, tools, generic, elements, objects, components, utilities).
- `@layer` позволяет управлять каскадом без повышения специфичности; порядок объявления слоёв определяет приоритет — позже объявленный слой побеждает.
- CSS Modules дают локальную инкапсуляцию классов через хэширование имён; CSS-in-JS удобен для динамических стилей, но может иметь runtime-накладные расходы — zero-runtime решения (Vanilla Extract) возвращают производительность CSS.
- Современный стек комбинирует custom properties для темизации, `@layer` для управления каскадом, CSS Modules/BEM для инкапсуляции и утилитарные классы для быстроты разработки.

## Заключение

CSS custom properties — runtime-механизм, делающий стили адаптируемыми без пересборки. Токенизация через три уровня (primitive, semantic, component) позволяет менять темы без правки компонентов. Методологии именования (BEM, CUBE, ITCSS) и `@layer` решают проблему масштабируемости, разбивая архитектуру стилей на предсказуемые слои. CSS Modules обеспечивают инкапсуляцию классов без ручного переименования. Все подходы дополняют друг друга: кастомные свойства, `@layer`, методология и инкапсуляция — разные инструменты для разных аспектов одной задачи.
