...
Formatting contexts — основа предсказуемого поведения CSS. BFC изолирует блоки, устраняет обтекание float и предотвращает выпадение margin'ов; `display: flow-root` — современный способ его создать без побочных эффектов. FFC и GFC добавляют свои правила поверх базового потока. Containing block определяет систему координат для `position: absolute/fixed`, и его неожиданное смещение (`transform` на предке) — одна из частых причин неверного позиционирования. Понимание этих концепций объясняет большинство «неожиданных» поведений в вёрстке.
