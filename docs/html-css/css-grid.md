...
order: 4
tags: ["grid", "fr", "minmax", "subgrid", "grid-template-areas"]
questions:
  - "Чем explicit grid отличается от implicit grid и как браузер создаёт неявные треки"
  - "Как `fr` распределяет доступное пространство и почему его поведение отличается от `%` или `px`"
  - "Как `minmax()` в сочетании с `auto-fit` и `auto-fill` создаёт адаптивные сетки без медиа-запросов"
  - "Как `grid-template-areas` описывает макет визуально и какие ограничения на форму областей"
  - "Чем `justify-items`/`align-items` отличается от `justify-content`/`align-content` в Grid"
  - "Какую проблему решает subgrid и почему без него сложно выровнять вложенные компоненты"
---
...
- Grid — двумерная система раскладки (строки + колонки одновременно); Flexbox — одномерная. Explicit grid — заданные явно треки; implicit grid — автоматически добавленные браузером через `grid-auto-*`.
- `fr` распределяет доступное пространство после фиксированных треков и gap'ов; в отличие от `%`, учитывает только свободное место, а не всю ширину контейнера.
- `minmax(min, max)` задаёт диапазон размеров трека; `repeat(auto-fit, minmax(250px, 1fr))` создаёт адаптивную сетку без медиа-запросов. `auto-fit` схлопывает пустые треки, `auto-fill` — оставляет.
- `grid-template-areas` описывает макет визуально (ASCII-диаграмма) и легко перестраивается через `@media`; имена областей должны формировать прямоугольник.
- `justify-items`/`align-items` выравнивают содержимое ячеек; `justify-content`/`align-content` — всю сетку в контейнере (когда сетка меньше контейнера).
- Subgrid позволяет вложенной сетке наследовать треки родителя, решая проблему выравнивания вложенных компонентов по общей сетке.

## Заключение

CSS Grid — двумерная система раскладки для сложных страничных структур. `fr` с `minmax()` и `auto-fit` позволяют создавать адаптивные сетки без медиа-запросов. `grid-template-areas` делает макет читаемым и легко перестраиваемым через `@media`. Subgrid решает давнюю проблему выравнивания вложенных компонентов по общей сетке. Grid и Flexbox дополняют друг друга: Grid — для страничного макета, Flexbox — для распределения по одной оси.
