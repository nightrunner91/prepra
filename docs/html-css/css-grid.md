...
questions:
  - "Чем explicit grid отличается от implicit grid?"
  - "Как единица `fr` распределяет пространство в сравнении с `%` или `px`?"
  - "Что делает функция `minmax()` и как она сочетается с `auto-fit`?"
  - "В чём разница между `auto-fit` и `auto-fill`?"
  - "Как работает `grid-template-areas` и какие ограничения на форму областей?"
  - "Как разместить элемент по линиям сетки и что означает `span`?"
  - "Чем `justify-items` отличается от `justify-content` в Grid?"
  - "Что такое subgrid и какую проблему он решает?"
  - "Что делает `grid-auto-flow: dense` и какие риски с этим связаны?"
  - "Почему `fr` может вести себя неожиданно при работе с контентом?"
---
