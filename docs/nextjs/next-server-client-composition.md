...
section: nextjs
description: "Главное правило Next.js App Router — **серверный компонент нельзя рендерить внутри клиентского**, но **клиентский компонент можно передать в серверный через `children`**. Этот документ разбирает вс..."
order: 10
tags: ["серверные", "клиентские", "компоненты", "правила", "композиции"]
---
