...
questions:
  ...
  - "Как передать серверный JSX в клиентский компонент, не нарушая правил"
  - "Что происходит с клиентским компонентом при сборке в RSC-потоке"
  - "Какой паттерн используется для чередования серверных и клиентских компонентов в дереве"
  - "Почему children в клиентском компоненте могут содержать серверный JSX"
  - "Чем Server Actions заменяют попытку передать callback через children"
  - "Какие данные нельзя передавать через children в RSC-потоке"
  - "Как избежать антипаттерна с `use client` на весь файл"
---

# Серверные и клиентские компоненты: правила композиции

Главное правило Next.js App Router — **серверный компонент нельзя рендерить внутри клиентского**, но **клиентский компонент можно передать в серверный через `children`**. Этот документ разбирает все допустимые и недопустимые комбинации с примерами. Понимание этих ограничений помогает уменьшить клиентский JavaScript и избегать ошибок сборки, а на практике — выбирать правильную границу между серверной и клиентской логикой.

## Содержание

1. [Базовые определения](#базовые-определения)
2. [Правило №1: children — это лазейка](#правило-1-children-это-лазейка)
3. [Правило №2: импорт клиентского в серверном](#правило-2-импорт-клиентского-в-серверном)
4. [Правило №3: импорт серверного в клиентском — ЗАПРЕЩЁН](#правило-3-импорт-серверного-в-клиентском-запрещён)
5. [Правило №4: серверный как children в клиентский](#правило-4-серверный-как-children-в-клиентский)
6. [Сводная таблица](#сводная-таблица)
7. [Паттерн «Interleaving» — чередование компонентов](#паттерн-interleaving-чередование-компонентов)
8. [Почему так работает: модель сериализации RSC](#почему-так-работает-модель-сериализации-rsc)
9. [Практические рецепты](#практические-рецепты)
...
- В Next.js App Router все компоненты по умолчанию серверные, а клиентские помечаются директивой `"use client"`.
- Серверный компонент может импортировать и рендерить клиентский компонент напрямую — в RSC-поток записывается ссылка на клиентский бандл.
- Клиентский компонент не может импортировать серверный, потому что серверный код не должен попадать в браузерный рантайм.
- Главный паттерн композиции — передача серверного JSX в клиентский компонент через `children` или произвольные пропсы типа `React.ReactNode`.
- `children`, переданные из серверного компонента, уже сериализованы в RSC-потоке, поэтому клиентский компонент просто вставляет готовые элементы, не исполняя их.
- Интерактивность следует опускать как можно ниже по дереву, а тяжёлые серверные вычисления — поднимать выше.
- Через клиентский компонент нельзя передавать несериализуемые данные: функции, классы, `Date` и `Symbols`; для callback'ов нужны Server Actions.
- Server Actions позволяют вызывать серверную логику из клиентского компонента, не импортируя серверный модуль.
- Динамический импорт и директива `"use client"` на весь файл не являются способом обойти ограничения композиции.

## Заключение

Правила композиции серверных и клиентских компонентов в Next.js сводятся к одной идее: серверный код выполняется только на сервере, а клиентский бандл получает лишь необходимый минимум интерактивности. Используйте `children` и пропсы типа `React.ReactNode`, чтобы пропускать серверный JSX сквозь клиентские обёртки, и применяйте Server Actions для обратной связи с сервером. Старайтесь держать клиентские компоненты ближе к листьям дерева, избегайте динамического импорта серверных модулей и не размазывайте `"use client"` на файлы, где интерактивность нужна лишь в небольшой части.
