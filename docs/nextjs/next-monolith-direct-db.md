...
questions:
  ...
  - "Как Server Component может обращаться к базе данных напрямую?"
  - "Почему важен singleton-паттерн при подключении PrismaClient в Next.js?"
  - "Какие плюсы даёт отказ от отдельного API-слоя?"
  - "Какие риски безопасности возникают при прямом доступе к БД и как их снижить?"
  - "Что такое connection pooling и почему это критично для serverless-деплоя?"
  - "Когда монолитный Next.js подходит, а когда лучше использовать отдельный API?"
  - "Какие навыки работы с БД нужны frontend-разработчику для коммерческой разработки в монолите?"
  - "Чем Prisma отличается от Drizzle и Kysely?"
  - "Почему после мутации в Server Action нужно вызывать `revalidatePath` или `revalidateTag`?"
---

# Next.js-монолит: прямой доступ к БД
...
1. [Что такое монолитный подход в Next.js](#что-такое-монолитный-подход-в-nextjs)
2. [Жизнеспособность в коммерческой разработке](#жизнеспособность-в-коммерческой-разработке)
3. [Архитектура прямого обращения к БД](#архитектура-прямого-обращения-к-бд)
4. [Плюсы подхода](#плюсы-подхода)
5. [Минусы подхода](#минусы-подхода)
6. [Подводные камни для frontend-разработчика](#подводные-камни-для-frontend-разработчика)
7. [Базы данных в монолитном Next.js](#базы-данных-в-монолитном-nextjs)
8. [ORM и инструменты работы с БД](#orm-и-инструменты-работы-с-бд)
9. [Необходимый уровень знаний для коммерческой разработки](#необходимый-уровень-знаний-для-коммерческой-разработки)
10. [Навыки для решения бизнес-задач](#навыки-для-решения-бизнес-задач)
11. [Паттерны и примеры](#паттерны-и-примеры)
12. [Когда стоит, а когда не стоит](#когда-стоит-а-когда-не-стоит)
13. [Сравнение с традиционной архитектурой](#сравнение-с-традиционной-архитектурой)
...
- Next.js-монолит позволяет Server Components и Server Actions обращаться к БД напрямую, исключая отдельный REST/GraphQL слой.
- Подход полностью жизнеспособен для стартапов, внутренних инструментов и небольших SaaS, но требует осторожности в высоконагруженных и микросервисных системах.
- В типичной структуре проекта `lib/db.ts` содержит singleton PrismaClient, а бизнес-логика выносится в `lib/services/` и Server Actions.
- Прямой доступ к БД даёт end-to-end типобезопасность, меньше шаблонного кода и меньше сетевых хопов, но привязывает бизнес-логику к Next.js.
- Безопасность децентрализована: каждый Server Component и Server Action должен сам проверять авторизацию и валидировать входные данные.
- Frontend-разработчику в монолите нужно понимать реляционную модель, миграции, транзакции, индексы, connection pooling и обработку ошибок БД.
- Для serverless-деплоев важны connection pooler (PgBouncer, Supavisor, Prisma Accelerate) и корректная настройка `PrismaClient`.
- Популярный стек: PostgreSQL + Prisma для типичных проектов; Neon/Supabase для serverless; Turso + Drizzle для edge.
- Service layer, оптимистичные обновления, пагинация, полнотекстовый поиск и multi-tenancy — основные паттерны работы с данными.
- Монолит не подходит, если нужен публичный API, мобильное приложение, независимое масштабирование или чёткое разделение frontend/backend команд.
- Гибридный подход с Next.js как BFF часто оптимален: часть данных читается напрямую, часть запрашивается из микросервисов.
...
- [Vercel Postgres — Connection Pooling](https://vercel.com/docs/storage/vercel-postgres/using-an-orm)
