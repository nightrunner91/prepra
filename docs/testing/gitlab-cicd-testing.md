...
tags:
  ...
  - "pipeline"
questions:
  - "Как устроен фронтенд-pipeline в GitLab CI и почему stages выполняются последовательно, а job'ы внутри stage — параллельно"
  - "Чем cache отличается от artifacts и как их комбинация ускоряет pipeline"
  - "Как параллелизация unit-тестов через parallel matrix и шардинг E2E сокращают время выполнения pipeline"
  - "Почему Playwright запускают в официальном Docker-образе и как services помогают тестировать с базами данных"
  - "Как needs и rules оптимизируют выполнение job'ов и когда запускать тяжёлые E2E-тесты"
  - "Какие форматы отчётов делают результаты тестов видимыми в MR и как настроить coverage"
  - "Почему secrets нельзя коммитить в репозиторий и как безопасно хранить их в CI/CD Variables"
---
...
- Фронтенд-pipeline строится из стадий `lint → typecheck → unit → build → e2e`; stages выполняются последовательно, а job'ы внутри stage — параллельно, что позволяет ускорить выполнение без потери порядка проверок.
- `cache` ускоряет установку зависимостей между pipeline'ами (например, `node_modules`), а `artifacts` передаёт файлы между job'ами внутри одного pipeline (например, сборку для E2E-тестов).
- Параллелизация unit-тестов через `parallel: matrix` с переменными `SHARD_INDEX` и `SHARD_TOTAL` распределяет нагрузку; шардинг E2E требует отдельного job'а для `merge-reports`, который собирает результаты из `blob-reports`.
- Playwright запускают в официальном Docker-образе с предустановленными браузерами и системными зависимостями; `services` поднимают базы данных или моки рядом с job'ом, обращаясь к ним по alias.
- `needs` строит DAG-зависимости, позволяя job'ам запускаться раньше без ожидания всей предыдущей stage; `rules` управляют условным запуском, например, тяжёлые E2E только в `main` или по label MR.
- JUnit-отчёты и регулярное выражение coverage делают результаты тестов видимыми в MR; это улучшает обратную связь и помогает быстро выявлять проблемы.
- Secrets хранят в CI/CD Variables (желательно protected), никогда не коммитят `.env` в репозиторий; полный `.gitlab-ci.yml` комбинирует кэш, artifacts, `needs`, шардинг, `rules` и отчёты в единый pipeline.
