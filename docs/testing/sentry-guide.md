...
questions:
  ...
  - "Почему порядок инициализации SDK критичен и как это связано с перехватом ошибок"
  - "Как source maps влияют на качество мониторинга и почему их нельзя публиковать публично"
  - "Что произойдёт, если не снизить tracesSampleRate в production и как это влияет на квоту"
  - "Как контекст и breadcrumbs превращают абстрактные ошибки в воспроизводимые сценарии"
  - "Какие данные нельзя отправлять в Sentry и как их фильтровать через beforeSend"
  - "Как AdBlock блокирует запросы к Sentry и какие есть решения этой проблемы"
---
...
- Sentry — платформа для мониторинга ошибок, производительности, сессий и логов в реальном времени; решает четыре задачи: Error Monitoring, Performance Monitoring, Session Replay и Logging.
- Инициализация Sentry должна быть первым импортом, иначе ошибки из ранних модулей не будут перехвачены; в React для перехвата ошибок рендеринга используются ErrorBoundary или хуки onUncaughtError/onCaughtError.
- Next.js требует отдельных конфигов для клиента, Node.js и Edge Runtime; source maps загружаются в Sentry при сборке и не публикуются публично.
- captureException используется для ошибок, captureMessage — для событий без исключения, Sentry.logger — для структурированных логов; tracesSampleRate в production должен быть снижен до 0.1 или ниже.
- Контекст (setUser, setTag, setExtra) и breadcrumbs превращают абстрактную ошибку в воспроизводимую историю; release tracking связывает ошибки с версиями приложения.
- AdBlock блокирует запросы к ingest.sentry.io, проблема решается через tunnel/tunnelRoute; конфиденциальные данные фильтруются через beforeSend или настройки dataCollection.
