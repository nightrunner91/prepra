...
questions:
  - "Что такое Sentry и какие задачи решает"
  - "Почему Sentry нужно инициализировать первым импортом"
  - "Как перехватывать ошибки рендеринга в React"
  - "Какие конфиги нужны для Next.js и почему три среды"
  - "Как настроить source maps и почему они важны"
  - "Чем отличается captureException от captureMessage"
  - "Как работает distributed tracing и зачем нужен tracePropagationTargets"
  - "Какие метрики показывает Release Health"
  - "Как решить проблему блокировки Sentry AdBlock"
  - "Какие данные нельзя отправлять в Sentry и как фильтровать"
---

# Sentry: интеграция и мониторинг ошибок

Production ломается не на машине разработчика, а у реальных пользователей с их устройствами, сетями и сценариями. Sentry помогает увидеть эти ошибки раньше, чем придёт жалоба в поддержку. В статье разберём, как встроить Sentry в React, Next.js, Vue и Nuxt, настроить source maps, мониторинг производительности и избежать типичных ошибок.
...
- Sentry — платформа для мониторинга ошибок, производительности, сессий и логов в реальном времени.
- Инициализация Sentry должна быть первым импортом в приложении, иначе ошибки из ранних модулей не будут перехвачены.
- В React для перехвата ошибок рендеринга используются `ErrorBoundary` (React 18) или хуки `onUncaughtError`/`onCaughtError` (React 19+).
- Next.js требует отдельных конфигов для клиента, Node.js и Edge Runtime.
- Source maps загружаются в Sentry при сборке, а не публикуются в публичном доступе.
- `captureException` используется для ошибок, `captureMessage` — для событий без исключения, `Sentry.logger` — для структурированных логов.
- `tracesSampleRate` в production должен быть снижен до `0.1` или ниже, чтобы не исчерпать квоту.
- Контекст (`setUser`, `setTag`, `setExtra`) и breadcrumbs превращают абстрактную ошибку в воспроизводимую историю.
- Release tracking связывает ошибки с версиями приложения и помогает находить регрессии.
- AdBlock блокирует запросы к `ingest.sentry.io`, проблема решается через `tunnel`/`tunnelRoute`.
- Конфиденциальные данные фильтруются через `beforeSend` или настройки `dataCollection`.

## Заключение

Sentry превращает пассивное «у меня работает» в активный мониторинг production. Ключевые моменты: инициализируйте SDK первым импортом, настройте source maps, снизьте sample rates для production, добавляйте контекст к ошибкам и настройте алерты. Не забывайте фильтровать PII и тестировать интеграцию с помощью тестовой ошибки. Хорошо настроенный Sentry экономит часы отладки и помогает находить проблемы до жалоб пользователей.
...
- [Sentry for Nuxt](https://docs.sentry.io/platforms/javascript/guides/nuxt/)
