...
order: 7
tags: ["testing-pyramid", "unit-tests", "integration-tests", "e2e-tests", "tdd", "code-coverage"]
questions:
  - "Какие уровни тестов входят в пирамиду Кона и в каком соотношении?"
  - "Чем Testing Trophy отличается от пирамиды и почему он лучше подходит для React-приложений?"
  - "Расшифруйте акроним FIRST и объясните каждый принцип."
  - "Как выглядит паттерн AAA и зачем он нужен?"
  - "Опишите цикл TDD Red-Green-Refactor и когда он избыточен."
  - "Что тестировать unit-тестами, а что — интеграционными и E2E?"
  - "Почему 100% code coverage не гарантирует отсутствия багов?"
  - "Какие антипаттерны делают тесты хрупкими и бесполезными?"
  - "Чем поведенческое тестирование отличается от тестирования реализации?"
  - "Как стоимость исправления бага меняется от этапа к этапу?"
---

# Основы тестирования: пирамида, принципы, стратегии

Каждое изменение кода несёт риск сломать уже работающий функционал, особенно в команде и на долгосрочном проекте. Автотесты снижают этот риск, но только если их правильно выбирать, писать и распределять по уровням. В этой статье разберём пирамиду и Testing Trophy, уровни unit/интеграционных/E2E тестов, принципы FIRST, паттерн AAA, TDD, code coverage и антипаттерны, которые стоит избегать.
...
- Тесты — это код, проверяющий код; их главная цель — уверенность при изменениях, а не покрытие ради процента.
- Unit-тесты быстрые, стабильные и дешёвые; они проверяют одну единицу кода в изоляции.
- Интеграционные тесты проверяют взаимодействие модулей и дают больше уверенности за счёт тестирования реального поведения, а не моков.
- E2E-тесты имитируют пользовательские сценарии в браузере; они самые дорогие, но дают максимальную уверенность.
- Пирамида Кона рекомендует ~70% unit, ~20% интеграционных и ~10% E2E; Testing Trophy смещает акцент на интеграционные тесты для React/Next.js.
- FIRST-принципы требуют, чтобы тесты были быстрыми, изолированными, повторяемыми, самопроверяемыми и своевременными.
- Паттерн AAA помогает структурировать тест на Arrange, Act, Assert; для очистки используется `afterEach`.
- TDD — это цикл Red-Green-Refactor, который форсирует дизайн кода и страховку перед рефакторингом.
- Покрытие кода — метрика, а не цель; важнее качество тестов и покрытие критичных веток.
- Тестируйте поведение, а не реализацию; не тестируйте фреймворки, сторонние библиотеки и тривиальный код.
- Smoke, regression, sanity и acceptance-тесты решают разные задачи: от быстрой проверки деплоя до защиты от возврата багов.
- Хрупкие селекторы, избыточное мокирование, snapshot всего и зависимость тестов друг от друга — распространённые антипаттерны.

## Заключение

1. Эффективная стратегия тестирования начинается с понимания уровней тестов и их стоимости.
2. Для серверного кода и библиотек хорошо работает пирамида Кона, а для React/Next.js — Testing Trophy с акцентом на интеграционные тесты.
3. Хороший тест следует FIRST, использует AAA и проверяет поведение, а не внутреннюю реализацию.
4. TDD помогает проектировать код и защищает от регрессий, но не обязателен для прототипов и исследовательских задач.
5. Покрытие кода полезно для поиска непротестированных участков, но не заменяет качество проверок.
6. Избегайте антипаттернов — хрупких селекторов, избыточных моков и snapshot-тестирования всего подряд.
7. Начните с покрытия критичной бизнес-логики unit-тестами, добавьте интеграционные тесты компонентов и завершите критичными E2E-сценариями.
...
- [MDN — Tools and testing](https://developer.mozilla.org/en-US/docs/Learn/Tools_and_testing)
