...
questions:
  ...
  - "Что произойдёт, если нарушить FIRST-принципы — например, писать медленные или зависимые друг от друга тесты"
  - "Как TDD Red-Green-Refactor форсирует дизайн кода и почему этот подход может быть избыточен для прототипов"
  - "Почему code coverage — это метрика, а не цель, и что важнее — процент покрытия или качество тестов"
  - "Что произойдёт, если тестировать реализацию, а не поведение, и как это влияет на устойчивость тестов к изменениям"
  - "Какие антипаттерны (хрупкие селекторы, избыточное мокирование, snapshot всего) делают тесты бесполезными и как их избежать"
---
...
- Тесты — это код, проверяющий код; их главная цель — уверенность при изменениях, а не покрытие ради процента. Тестируйте поведение, а не реализацию.
- Пирамида Кона (70% unit, 20% интеграционных, 10% E2E) и Testing Trophy (акцент на интеграционные для React/Next.js) определяют баланс: unit-тесты быстры и дёшевы, интеграционные проверяют реальное взаимодействие модулей, E2E имитирует пользователя, но дороги.
- FIRST-принципы (Fast, Isolated, Repeatable, Self-validating, Timely) и паттерн AAA (Arrange-Act-Assert) делают тесты читаемыми, стабильными и поддерживаемыми.
- TDD — цикл Red-Green-Refactor: сначала тест, потом код, потом рефакторинг; форсирует дизайн и защищает от регрессий, но избыточен для прототипов и исследовательских задач.
- Покрытие кода — метрика, а не цель; важнее качество тестов и покрытие критичных веток бизнес-логики, а не 100% всех строк.
- Smoke, regression, sanity и acceptance-тесты решают разные задачи; избегайте антипаттернов — хрупких селекторов, избыточных моков, snapshot всего и зависимости тестов друг от друга.
