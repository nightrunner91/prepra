...
questions:
  ...
  - "В чём разница между stub, spy и mock?"
  - "Когда использовать vi.fn(), а когда vi.spyOn()?"
  - "Зачем нужен vi.hoisted() при работе с vi.mock()?"
  - "Как MSW отличается от vi.mock() по уровню мокирования?"
  - "Почему важно вызывать server.resetHandlers() в afterEach?"
  - "Когда нужен vi.useFakeTimers() и как вернуть реальные таймеры?"
  - "В чём разница между clearAllMocks, resetAllMocks и restoreAllMocks?"
  - "Зачем использовать фабрики тестовых данных вместо хардкода?"
  - "Почему onUnhandledRequest: 'error' рекомендуется в тестах с MSW?"
---

# Мокирование и изоляция тестов

Без изоляции от внешних зависимостей тесты становятся медленными, хрупкими и зависят от сети, базы данных или текущего времени. Мокирование позволяет заменить эти зависимости контролируемыми двойниками и проверять поведение кода в предсказуемых условиях. В этой статье разберём инструменты Vitest — от `vi.fn()` до `vi.mock()` и `vi.hoisted()`, — а также MSW, мокирование браузерных API, таймеров и тестовых данных.
...
18. [Ключевые тезисы для интервью](#ключевые-тезисы-для-интервью)
...
- Мокируйте только внешние зависимости: API, таймеры, браузерные API и storage; внутренние модули тестируйте как есть.
- Stub возвращает фиксированное значение, spy отслеживает вызовы и сохраняет поведение, mock полностью заменяет реализацию.
- `vi.fn()` создаёт mock-функцию с нуля; `vi.spyOn()` оборачивает существующий метод и требует `mockRestore()`.
- `vi.mock()` полностью заменяет модуль и поднимается в начало файла; для переменных внутри factory используйте `vi.hoisted()`.
- MSW перехватывает запросы на сетевом уровне, поэтому работает с fetch, axios, react-query без изменения кода.
- Базовые MSW-handlers переиспользуются между тестами, а `server.use()` позволяет точечно переопределять поведение для одного теста.
- `server.resetHandlers()` в `afterEach` предотвращает «утечку» переопределений между тестами.
- Ручное мокирование `fetch` оправдано только в простых случаях; в остальном предпочтительнее MSW.
- `vi.useFakeTimers()` и `vi.setSystemTime()` делают тесты с таймерами и датами детерминированными.
- `clearAllMocks` очищает историю, `resetAllMocks` сбрасывает ещё и реализацию, `restoreAllMocks` восстанавливает оригиналы spy.
- Фабрики тестовых данных снижают дублирование и хрупкость тестов при изменении типов.
- Каждый тест должен быть независимым: не полагайтесь на порядок выполнения и всегда очищайте моки.

## Заключение

Мокирование — необходимый инструмент изоляции, но каждый mock отдаляет тест от реальности. Выбирайте минимально достаточный уровень: MSW для сетевых запросов, `vi.mock()` для модулей, `vi.spyOn()` для методов и `vi.fn()` для колбэков. Не забывайте очищать состояние моков между тестами, использовать фабрики данных и проверять поведение, а не реализацию. Регулярная практика с этими инструментами поможет писать быстрые, стабильные и уверенные тесты.

## Полезные ссылки

- [Vitest — Mocking](https://vitest.dev/guide/mocking.html)
- [Vitest — vi](https://vitest.dev/api/vi.html)
- [MSW Documentation](https://mswjs.io/docs/)
- [MDN — Service Worker API](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API)
- [Jest — Manual Mocks](https://jestjs.io/docs/manual-mocks) (концепции близки к Vitest)
