...
questions:
  ...
  - "Как выбрать между vi.fn(), vi.spyOn() и vi.mock() в зависимости от уровня изоляции"
  - "Как MSW отличается от vi.mock() и почему сетевое мокирование предпочтительнее ручного"
  - "Что произойдёт, если не очищать состояние моков между тестами, и как server.resetHandlers() и afterEach предотвращают утечки"
  - "Когда использовать vi.useFakeTimers() и как это связано с детерминированностью тестов"
  - "Зачем использовать фабрики тестовых данных и как они снижают хрупкость тестов"
  - "Какие антипаттерны мокирования (избыточные моки, зависимость от порядка, неочищенные моки) делают тесты хрупкими"
---
...
- Мокируйте только внешние зависимости (API, таймеры, браузерные API, storage); внутренние модули тестируйте как есть.
- Stub возвращает фиксированное значение, spy отслеживает вызовы и сохраняет поведение, mock полностью заменяет реализацию; `vi.fn()` создаёт mock-функцию с нуля, `vi.spyOn()` оборачивает существующий метод.
- `vi.mock()` полностью заменяет модуль и поднимается в начало файла; для переменных внутри factory используйте `vi.hoisted()`.
- MSW перехватывает запросы на сетевом уровне, поэтому работает с fetch, axios, react-query без изменения кода; базовые handlers переиспользуются, а `server.use()` позволяет точечно переопределять поведение.
- `vi.useFakeTimers()` и `vi.setSystemTime()` делают тесты с таймерами и датами детерминированными; `clearAllMocks` очищает историю, `resetAllMocks` сбрасывает реализацию, `restoreAllMocks` восстанавливает оригиналы spy.
- Фабрики тестовых данных снижают дублирование и хрупкость; каждый тест должен быть независимым, не полагаться на порядок выполнения и всегда очищать моки в `afterEach`.
