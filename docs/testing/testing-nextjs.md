...
questions:
  - "Как настроить Vitest для тестирования Next.js-компонентов"
  - "Чем тестирование Server Components отличается от клиентских компонентов"
  - "Как протестировать Server Action с валидацией и побочными эффектами"
  - "Какие подходы используются для тестирования Route Handlers"
  - "Как замокировать хуки из next/navigation в клиентских тестах"
  - "Как тестировать middleware с разными cookies и путями"
  - "Зачем использовать MSW вместо ручного мокирования fetch"
  - "Какие антипаттерны стоит избегать при тестировании Next.js"
---
...
- Next.js добавляет серверные абстракции, поэтому тестировать нужно бизнес-логику, а не внутренний механизм фреймворка.
- Vitest настраивается с `environment: "jsdom"`, глобальными матчерами и алиасами для работы с React и JSX.
- Клиентские компоненты тестируются через Testing Library как обычные React-компоненты, проверяя поведение, а не состояние.
- Server Components вызываются как async-функции, полученный JSX рендерится через Testing Library для проверки результата.
- Server Actions тестируются как unit-функции с моками зависимостей, включая проверку валидации и побочных эффектов.
- Route Handlers удобно тестировать прямым вызовом функций GET/POST с моком Request.
- Middleware — это чистая функция над NextRequest/NextResponse, которую можно проверить без запуска сервера.
- `next/navigation` и `next/headers` мокируются через `vi.mock`, чтобы эмулировать роутинг, cookies и заголовки.
- `revalidatePath` и `revalidateTag` проверяются через моки, а не через реальное поведение кэша.
- MSW перехватывает HTTP-запросы на уровне сети и даёт более реалистичное мокирование, чем `global.fetch`.
- App Router тестирует Server Components напрямую, а Pages Router — через `getServerSideProps`/`getStaticProps`.
- Базы данных, аутентификация и метаданные тестируются через моки соответствующих API без подключения к реальным сервисам.
