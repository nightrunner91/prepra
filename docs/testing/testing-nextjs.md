...
questions:
  - "Почему в Next.js тестировать нужно бизнес-логику, а не внутренний механизм фреймворка"
  - "Как тестирование Server Components отличается от клиентских компонентов и почему это важно"
  - "Как протестировать Server Action с валидацией и побочными эффектами, не запуская сервер"
  - "Как тестировать Route Handlers и Middleware как чистые функции и почему это предпочтительнее интеграционных тестов"
  - "Как мокировать next/navigation и next/headers для эмуляции роутинга, cookies и заголовков"
  - "Почему MSW предпочтительнее ручного мокирования fetch и как это влияет на реалистичность тестов"
  - "Какие антипаттерны (тестирование реализации, подключение к реальным БД, игнорирование моков) делают тесты Next.js хрупкими"
---

# Тестирование в Next.js

Next.js объединяет серверную и клиентскую логику в одном приложении, поэтому тестирование здесь охватывает сразу несколько слоёв: от клиентских компонентов и Server Components до API-эндпоинтов и Middleware. В статье разберём практические подходы к настройке тестового окружения на Vitest, мокированию специфичных для Next.js API и проверке типичных сценариев. Вы узнаете, как тестировать бизнес-логику, не уходя в детали внутреннего устройства фреймворка.
...
19. [Ключевые тезисы для интервью](#ключевые-тезисы-для-интервью)
...
- Next.js добавляет серверные абстракции, поэтому тестировать нужно бизнес-логику, а не внутренний механизм фреймворка; Vitest настраивается с `environment: "jsdom"`, глобальными матчерами и алиасами для React и JSX.
- Клиентские компоненты тестируются через Testing Library как обычные React-компоненты (проверяя поведение, а не состояние); Server Components вызываются как async-функции, полученный JSX рендерится для проверки результата.
- Server Actions тестируются как unit-функции с моками зависимостей (проверка валидации и побочных эффектов); Route Handlers вызываются прямым вызовом GET/POST с моком Request; Middleware — чистая функция над NextRequest/NextResponse, проверяется без запуска сервера.
- `next/navigation` и `next/headers` мокируются через `vi.mock` для эмуляции роутинга, cookies и заголовков; `revalidatePath` и `revalidateTag` проверяются через моки, а не реальное поведение кэша.
- MSW перехватывает HTTP-запросы на уровне сети, давая более реалистичное мокирование, чем `global.fetch`; App Router тестирует Server Components напрямую, Pages Router — через `getServerSideProps`/`getStaticProps`.
- Базы данных, аутентификация и метаданные тестируются через моки соответствующих API без подключения к реальным сервисам; избегайте антипаттернов — тестирования реализации, подключения к реальным БД, игнорирования очистки моков.

## Заключение

Тестирование Next.js требует различных подходов для клиентских и серверных частей приложения. Главное правило — проверять бизнес-логику и поведение, а не детали реализации фреймворка. Правильно настроенный Vitest с моками `next/navigation`, `next/headers` и `next/cache` покрывает большинство сценариев. Для HTTP-запросов предпочитайте MSW ручному мокированию `fetch`, а Server Actions и Route Handlers тестируйте как обычные функции. Соблюдение этих принципов делает тесты быстрыми, стабильными и полезными при рефакторинге.
