...
questions:
  ...
  - "В каких сценариях использовать `getByRole`, `getByLabelText` и `getByTestId`, и почему приоритет именно такой"
  - "Что произойдёт, если использовать `getBy*` для проверки отсутствия элемента, и почему `queryBy*` — правильный выбор"
  - "Как `userEvent` отличается от `fireEvent` и почему симуляция реальных действий пользователя важна для тестов"
  - "Как тестировать асинхронные операции (загрузка данных, условный рендеринг) и почему MSW предпочтительнее ручного мокирования"
  - "Какие антипаттерны (зависимость тестов, мокирование хуков, проверка innerHTML) делают тесты хрупкими и как их избежать"
---

# Тестирование React-компонентов с React Testing Library

Большинство багов в React-приложениях проявляется не внутри хуков, а в том, что пользователь видит и с чем взаимодействует. React Testing Library ставит во главу угла именно этот взгляд: тесты проверяют DOM, события и доступность, а не внутреннее состояние компонентов. В статье разберём, как настроить окружение, выбирать query-методы, моделировать пользовательские действия и избегать хрупких проверок.
...
- React Testing Library тестирует поведение пользователя (DOM, события, доступность), а не внутреннее состояние компонентов; это делает тесты устойчивыми к рефакторингу.
- Query-методы: `getByRole` (приоритетный, опирается на семантику) > `getByLabelText` > `getByTestId`; `getBy*` — элемент должен быть, `queryBy*` — проверка отсутствия, `findBy*` — асинхронное появление.
- `userEvent` симулирует реальные действия пользователя (клик, ввод текста) и предпочтительнее `fireEvent`; `fireEvent` — низкоуровневый инструмент для событий, которые `userEvent` не покрывает.
- Асинхронные операции тестируются через `findBy*`, `waitFor` или `waitForElementToBeRemoved`; MSW имитирует сетевые запросы реалистичнее ручного мокирования `fetch`.
- Компоненты с контекстом, роутером или Redux оборачиваются в провайдеры через кастомный `render`; `renderHook` изолирует тестирование сложной логики хуков без UI-обёрток.
- Антипаттерны: зависимость тестов друг от друга, мокирование внутренних хуков React, проверка `container.innerHTML`, использование `getBy*` для проверки отсутствия элемента.

## Заключение

React Testing Library смещает фокус с внутреннего устройства компонентов на то, что видит и делает пользователь. Освоив `getByRole`, `userEvent`, `findBy*` и кастомные `render`-обёртки, вы сможете писать тесты, которые не ломаются при рефакторинге и действительно повышают уверенность в коде. Не забывайте очищать моки и DOM между тестами, мокировать внешние зависимости через MSW и избегать проверки деталей реализации. Практикуйтесь на реальных компонентах — именно в повторяющихся сценариях формируется интуиция выбора правильного query.

## Полезные ссылки

- [React Testing Library — документация](https://testing-library.com/docs/react-testing-library/intro/)
- [jest-dom — кастомные матчеры](https://testing-library.com/docs/ecosystem-jest-dom/)
- [user-event — документация](https://testing-library.com/docs/user-event/intro/)
- [MSW — Mock Service Worker](https://mswjs.io/)
- [Vitest — руководство по тестированию](https://vitest.dev/guide/)
- [Приоритеты query в Testing Library](https://testing-library.com/docs/queries/about/#priority)
