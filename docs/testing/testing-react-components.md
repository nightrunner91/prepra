...
questions:
  - "Почему React Testing Library считается предпочтительнее Enzyme?"
  - "Какой query-метод имеет высший приоритет и почему?"
  - "В чём разница между getBy*, queryBy* и findBy*?"
  - "Когда использовать userEvent, а когда fireEvent?"
  - "Как правильно тестировать асинхронную загрузку данных?"
  - "Зачем нужны кастомные render-обёртки с провайдерами?"
  - "Для чего используется renderHook?"
  - "Какие ошибки чаще всего делают тесты хрупкими?"
---
...
- React Testing Library тестирует поведение пользователя, а не внутреннюю реализацию компонента.
- `getByRole` — приоритетный query, потому что опирается на семантику и доступность.
- `getBy*` используется, когда элемент должен быть; `queryBy*` — для проверки отсутствия; `findBy*` — для асинхронного появления.
- `userEvent` симулирует реальные действия пользователя и предпочтительнее `fireEvent` в большинстве сценариев.
- `fireEvent` остаётся низкоуровневым инструментом для событий, которые `userEvent` не покрывает.
- Асинхронные операции тестируются через `findBy*`, `waitFor` или `waitForElementToBeRemoved`.
- MSW предпочтительнее ручного мокирования `fetch`, так как имитирует сетевые запросы реалистичнее.
- Компоненты с контекстом, роутером или Redux оборачиваются в провайдеры через кастомный `render`.
- `renderHook` изолирует тестирование сложной логики хуков без UI-обёрток.
- Проверка отсутствия элемента всегда делается через `queryBy*`, а не `getBy*` внутри `not.toBeInTheDocument`.
- Тесты не должны зависеть друг от друга, мокировать внутренние хуки React или проверять `container.innerHTML`.
