...
questions:
  ...
  - "Как выбрать правильный matcher для разных типов данных и почему это важно для читаемости тестов"
  - "Как мокировать функции, методы объекта и целые модули в Vitest, и когда что использовать"
  - "Зачем нужны фейковые таймеры и как они помогают тестировать асинхронный код без реального ожидания"
  - "Какие плюсы и минусы snapshot-тестирования и когда его стоит применять"
  - "Как тестировать React-хуки и побочные эффекты, и почему важна обёртка в act"
  - "Что показывает code coverage и почему высокий процент не гарантирует отсутствие багов"
  - "Какие антипаттерны (зависимость тестов, неочищенные моки, snapshot всего) делают тесты хрупкими и как их избежать"
---

# Unit-тестирование с Vitest

Без автоматических тестов рефакторинг превращается в игру в угадывание: сломалось ли что-то ещё или только то, что видно на экране. Vitest решает эту проблему для проектов на Vite, предлагая Jest-подобный API без дополнительных трансформеров и сложной конфигурации. В статье разберём установку, базовые проверки, мокирование, асинхронные сценарии, снапшоты и лучшие практики, которые делают тесты надёжными.
...
12. [Конфигурация для React, Next.js, Vue и Nuxt](#конфигурация-для-react-nextjs-vue-и-nuxt)
...
- Vitest — Jest-совместимый фреймворк для Vite-проектов с нативной поддержкой ESM и TypeScript; базовый API (`describe`/`it`/`expect`) и матчеры (`toBe` для примитивов, `toEqual` для объектов, `toBeCloseTo` для float, `toThrow` для ошибок) делают тесты читаемыми.
- `vi.fn()` создаёт mock-функцию, `vi.spyOn()` следует за методом, `vi.mock()` подменяет целый модуль; выбирайте инструмент в зависимости от того, нужно ли отслеживать вызовы или полностью заменить поведение.
- Фейковые таймеры (`vi.useFakeTimers`) контролируют `setTimeout`, `setInterval` и `Date` без реального ожидания; `it.each` запускает один тест с разными данными для покрытия edge cases.
- Snapshot-тесты фиксируют разметку, но обновлять их следует только после ручной проверки diff'а; не снимайте snapshot со всего подряд — это создаёт шум при изменениях.
- Хуки тестируются через `renderHook` с обёрткой побочных эффектов в `act`; разделяйте тесты с исходным кодом, очищайте моки в `afterEach`, используйте MSW для HTTP-заглушек.
- Покрытие кода измеряет затронутые строки, но высокий процент не гарантирует отсутствие багов; важнее качество тестов и покрытие критичных веток бизнес-логики.

## Заключение

Vitest снижает порог входа в тестирование для Vite-экосистемы, сохраняя привычный Jest-совместимый API. В статье мы разобрали базовые проверки, мокирование зависимостей, асинхронные сценарии, фейковые таймеры и подходы к тестированию хуков. Главное помнить: тесты должны быть изолированными, читаемыми и поддерживаемыми. Начните с настройки `vitest.config.ts`, добавьте setup-файл с очисткой моков и покрытие для CI — и вы получите уверенность при каждом рефакторинге.
