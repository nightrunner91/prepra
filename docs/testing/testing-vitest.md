...
questions:
  ...
  - "Как настроить окружение jsdom и setup-файл для React-компонентов"
  - "Когда использовать toBe, toEqual, toHaveBeenCalledWith и toThrow"
  - "Как мокировать функции, методы объекта и целые модули в Vitest"
  - "Зачем нужны фейковые таймеры и как продвигать время в тестах"
  - "Какие плюсы и минусы snapshot-тестирования"
  - "Как писать параметризованные тесты с it.each"
  - "Как тестировать React-хуки через renderHook и act"
  - "Что показывает code coverage и какие пороги ставить"
  - "Какие лучшие практики помогают поддерживать тесты в чистоте"
---
...
- Vitest — Jest-совместимый фреймворк для Vite-проектов с нативной поддержкой ESM и TypeScript.
- `describe` группирует тесты, `it`/`test` описывает сценарий, `expect` выполняет проверку через matcher'ы.
- Для примитивов используйте `toBe`, для объектов и массивов — `toEqual`, для float — `toBeCloseTo`, для ошибок — `toThrow`.
- `vi.fn()` создаёт mock-функцию, `vi.spyOn()` следует за методом, `vi.mock()` подменяет целый модуль.
- Фейковые таймеры (`vi.useFakeTimers`) позволяют контролировать `setTimeout`, `setInterval` и `Date` без реального ожидания.
- Snapshot-тесты фиксируют разметку, но обновлять их следует только после ручной проверки diff'а.
- `it.each` запускает один тест с разными данными, превращая edge cases в отдельные именованные проверки.
- Хуки тестируются через `renderHook`, а побочные эффекты оборачиваются в `act`.
- Покрытие кода измеряет затронутые строки, но высокий процент не гарантирует отсутствие багов.
- Разделяйте тесты с исходным кодом, очищайте моки в `afterEach` и используйте MSW для HTTP-заглушек.
