...
tags:
  ...
  - "snapshot-testing"
questions:
  - "Что означает принцип FIRST для тестов?"
  - "Как устроена структура AAA в тесте?"
  - "Как правильно именовать тесты?"
  - "Почему один тест должен проверять только одну концепцию?"
  - "Зачем использовать фабрики тестовых данных?"
  - "Какой приоритет селекторов рекомендует Testing Library?"
  - "Что такое Page Object Model и зачем он нужен в E2E-тестах?"
  - "Почему тесты не должны зависеть от порядка выполнения?"
  - "Что следует мокировать в тестах, а что нет?"
  - "Что делает тест детерминированным?"
---
...
- Хороший тест соответствует принципу FIRST: быстрый, изолированный, повторяемый, самопроверяемый и своевременный.
- Структура AAA делит тест на Arrange, Act и Assert, что упрощает чтение и отладку.
- Имя теста должно описывать действие, ожидаемый результат и условие выполнения.
- Каждый тест проверяет одну концепцию, а не цепочку несвязанных сценариев.
- Тестируй поведение, видимое пользователю, а не внутреннее состояние компонентов.
- Фабрики тестовых данных сокращают хрупкость тестов и подчеркивают значимые поля.
- Приоритет селекторов: getByRole, getByLabelText, getByTestId как последний резерв.
- Page Object Model выносит селекторы и шаги E2E в отдельные классы.
- Тесты должны быть изолированы и не зависеть от shared state или порядка запуска.
- Мокировать следует только внешние зависимости и границы, а не внутренние модули.
- Детерминированные тесты не зависят от текущего времени, сети и случайных данных.
- Snapshot-тесты полезны только для стабильных структур и требуют осознанного ревью.
