...
questions:
  - "Чем Playwright отличается от Cypress в поддержке браузеров и параллельного запуска"
  - "Почему семантические локаторы предпочтительнее CSS-селекторов"
  - "Как работает автоожидание в Playwright при действиях и ассертах"
  - "Как правильно дождаться сетевого ответа в тесте"
  - "Что такое Page Object Model и когда его стоит применять"
  - "Как ускорить тесты с аутентификацией через storageState"
  - "Какие опасности связаны с мокированием сетевых запросов"
  - "Для чего используется toHaveScreenshot и какие области лучше замаскировать"
  - "Почему не стоит использовать waitForTimeout"
  - "Как настроить параллельный запуск и retries в CI"
---

# E2E-тестирование с Playwright

Playwright позволяет автоматизировать браузерные сценарии в Chromium, Firefox и WebKit из единого API. Статья разбирает установку, основные методы поиска элементов, действия, ожидания, моки, визуальные тесты и организацию кода через Page Object Model. Вы узнаете, как сделать тесты стабильными, быстрыми и удобными в поддержке.
...
- Playwright поддерживает Chromium, Firefox и WebKit через единый API и изолированные контексты для каждого теста.
- Локаторы в Playwright работают лениво: элемент ищется в момент действия, а не при создании локатора.
- Семантические локаторы (`getByRole`, `getByLabel`) устойчивее CSS-селекторов и одновременно проверяют доступность.
- Действия в Playwright автоматически ждут видимости, стабильности и отсутствия перекрытия элемента.
- `expect`-assertions в Playwright — это retry-циклы, повторяющие проверку до таймаута.
- `waitForResponse` используется для надёжного ожидания сетевых запросов, а `waitForTimeout` — антипаттерн.
- `page.route` позволяет мокировать, изменять и прерывать сетевые запросы.
- `toHaveScreenshot` сравнивает скриншоты pixel-by-pixel, но динамичные области нужно маскировать.
- Page Object Model централизует локаторы и действия страницы, упрощая поддержку тестов.
- `storageState` и setup project позволяют переиспользовать аутентификацию между тестами.
- `fullyParallel` и `retries: 2` в CI ускоряют прогон и снижают влияние flaky-тестов.
- Тесты должны быть независимыми: каждый создаёт свои данные и не зависит от других.

## Заключение

Playwright предоставляет полный инструментарий для E2E-тестирования: от семантических локаторов и автоожиданий до моков, визуальных сравнений и CI-интеграции. Ключ к стабильным тестам — правильные локаторы, независимые сценарии и осознанное использование моков. Page Object Model и глобальная аутентификация через `storageState` делают прогоны быстрее и код чище. Не злоупотребляйте таймаутами и retries: они скрывают проблемы, а не решают их. Начните с базовой конфигурации, покройте критические пользовательские сценарии и постепенно наращивайте покрытие.
