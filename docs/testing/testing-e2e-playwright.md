...
questions:
  ...
  - "Почему семантические локаторы (getByRole, getByLabel) предпочтительнее CSS-селекторов и как они связаны с доступностью"
  - "Что произойдёт, если мокировать сетевые запросы, и какие опасности это создаёт для реалистичности тестов"
  - "Как Page Object Model и storageState ускоряют E2E-тесты и упрощают их поддержку"
  - "Как организовать независимые E2E-тесты (изоляция данных, параллельный запуск, retries в CI) и почему это критично для стабильности"
  - "Какие антипаттерны (waitForTimeout, хрупкие селекторы, зависимость тестов) делают E2E-тесты нестабильными и как их избежать"
---
...
- Семантические локаторы (`getByRole`, `getByLabel`) устойчивее CSS-селекторов и одновременно проверяют доступность; действия автоматически ждут видимости, стабильности и отсутствия перекрытия элемента.
- `page.route` позволяет мокировать сетевые запросы, но избыточное мокирование снижает реалистичность; `waitForResponse` надёжнее `waitForTimeout` (антипаттерн) для ожидания сетевых операций.
