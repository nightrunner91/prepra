---
title: "Оптимизация и ускорение E2E-тестов"
section: testing
order: 1
description: "Как сократить время E2E-тестов в CI без потери качества: параллелизм, шардинг, подготовка состояния и стабильные селекторы."
tags: ["e2e-оптимизация", "playwright", "ci", "шардинг", "storagestate", "flaky-тесты"]
questions:
  - "Почему E2E-тесты часто становятся узким местом в CI?"
  - "Как настроить параллельный запуск и шардинг в Playwright?"
  - "Как подготовить аутентификацию и тестовые данные через API/storageState?"
  - "Зачем мокировать внешние сервисы и какие селекторы использовать?"
  - "Почему `waitForTimeout` вреден и чем его заменить?"
  - "Как разделять тесты тегами и запускать только нужный набор?"
  - "Как правильно работать с flaky-тестами и retries?"
  - "Почему E2E лучше запускать против production-сборки?"
  - "Что такое селективный запуск и мониторинг времени прогона?"
  - "Какие пункты включены в чеклист оптимизации E2E?"
---

# Оптимизация и ускорение E2E-тестов

E2E-тесты дают максимальную уверенность, но длинный прогон в CI сводит их пользу на нет. В этой статье разберём, как ускорить E2E без потери качества: распараллелим тесты, подготовим состояние через API, замокаем внешние зависимости и настроим селективный запуск. Вы получите чеклист, который можно применить к своему проекту сегодня.
...
- E2E-тесты дороги из-за запуска браузера, навигации, UI-взаимодействий и ожиданий, поэтому ускорять нужно и отдельные тесты, и весь набор.
- `fullyParallel` и правильное число `workers` позволяют распараллелить тесты внутри одного runner'а, не рискуя изоляцией.
- Шардинг делит тесты между несколькими CI-раннерами, а blob-отчёты помогают сбалансировать нагрузку и объединить результаты.
- Аутентификацию и тестовые данные лучше готовить через API или seed, а не кликать по UI в каждом тесте.
- `storageState` и setup-проект в Playwright позволяют один раз залогиниться и переиспользовать сессию во всех тестах.
- Внешние сервисы стоит блокировать или мокировать через `page.route`, чтобы избежать сетевых задержек и нестабильности.
- Стабильные селекторы — роли, label и `data-testid` — снижают flaky rate и экономят время на перезапуски.
- Фиксированные `waitForTimeout` / `sleep` заменяются на явные ожидания состояния, сетевых ответов или исчезновения элементов.
- Теги `@smoke`, `@regression`, `@slow` дают возможность запускать только нужный поднабор тестов на каждом этапе CI.
- Retries нужны в CI, но системно падающие тесты следует чинить по trace, а не маскировать повторными попытками.
- E2E стоит запускать против production-сборки, потому что dev-сервер медленнее из-за компиляции на лету.
- Селективный запуск и регулярный мониторинг времени прогона помогают удерживать E2E-набор быстрым и полезным.

---

## Заключение

Длинные E2E-прогоны неизбежны, если запускать всё последовательно, логиниться через UI и ждать фиксированных пауз. Рост времени CI убивает доверие к тестам, поэтому оптимизация — часть качества, а не производительности. Комбинация параллелизма, шардинга, подготовки состояния через API, моков внешних сервисов и стабильных селекторов даёт ощутимый прирост скорости. Дополните это тегами, селективным запуском и мониторингом — и E2E снова станут настоящей защитой продукта, а не узким местом.
