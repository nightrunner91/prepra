...
questions:
  - "Как баланс между скоростью и качеством E2E-тестов влияет на доверие к CI и как оптимизация помогает его восстановить"
  - "Почему подготовка состояния через API и переиспользование аутентификации критичны для скорости E2E-тестов"
  - "Что произойдёт, если использовать фиксированные задержки вместо явных ожиданий, и как это влияет на стабильность тестов"
  - "Как параллелизм и шардинг дополняют друг друга в ускорении E2E-прогонов в CI"
  - "Почему селективный запуск и мониторинг времени прогона важнее, чем просто добавление retries"
  - "Какие антипаттерны (нестабильные селекторы, мокирование всего подряд, запуск против dev-сервера) делают E2E-тесты медленными и нестабильными"
---
...
- E2E-тесты дороги из-за запуска браузера, навигации и UI-взаимодействий; ускорение требует оптимизации как отдельных тестов (через API, моки, стабильные селекторы), так и всего набора (параллелизм, шардинг).
- Подготовка состояния через API и переиспользование аутентификации (`storageState`, setup-проект) критичны для скорости; внешние сервисы блокируют или мокируют через `page.route`, чтобы избежать сетевых задержек.
- Фиксированные задержки (`waitForTimeout`) заменяют на явные ожидания состояния, сетевых ответов или исчезновения элементов; стабильные селекторы (роли, label, `data-testid`) снижают flaky rate.
- Параллелизм (`fullyParallel`, `workers`) ускоряет тесты внутри одного runner'а; шардинг делит тесты между несколькими CI-раннерами, а blob-отчёты объединяют результаты.
- Селективный запуск через теги (`@smoke`, `@regression`) и мониторинг времени прогона важнее retries; системно падающие тесты чинят по trace, а не маскируют повторными попытками.
- E2E запускают против production-сборки (dev-сервер медленнее из-за компиляции); антипаттерны — нестабильные селекторы, избыточное мокирование, фиксированные задержки, отсутствие мониторинга.
