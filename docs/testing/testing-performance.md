...
questions:
  ...
  - Почему performance budgets и thresholds должны быть щедрыми, и как это влияет на стабильность тестов в CI?
  - Как k6 моделирует реалистичную нагрузку на API и какие инструменты помогают избежать flaky-тестов?
  - Почему профилирование React/Vue в тестах даёт только baseline, а не полную картину производительности?
  - Какие антипаттерны делают тестирование производительности бесполезным и как их избежать?
---
...
- Тестирование производительности охватывает загрузку страницы (Lighthouse CI), размер бандла (size-limit), отзывчивость интерфейса (Core Web Vitals) и нагрузку на API (k6).
- Lighthouse CI автоматизирует проверки в CI/CD; Core Web Vitals (LCP, INP, CLS) в headless-тестах используют как baseline, а не абсолютные значения.
- size-limit проверяет реальный размер бандла после tree-shaking и сжатия; performance budgets фиксируют лимиты на метрики и размеры чанков, останавливая регрессии на этапе PR.
- k6 моделирует виртуальных пользователей с stages и thresholds для реалистичной нагрузки; timing-тесты требуют щедрых порогов, чтобы не быть flaky.
- React/Vue Profiler выявляет грубые регрессии рендера, но добавляет overhead и не заменяет production-профилирование; production-мониторинг через web-vitals собирает реальные метрики пользователей.
- Антипаттерны: игнорирование производительности, нереалистичные бюджеты (score = 1.0), тестирование только в идеальных условиях, оптимизация без измерений.
