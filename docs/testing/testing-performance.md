...
questions:
  - Чем отличаются синтетические метрики Lighthouse от RUM в production?
  - Какие три Core Web Vitals стоит измерять в E2E-тестах?
  - Зачем в k6 добавлять sleep между запросами?
  - Как задать performance budget для размера бандла в CI?
  - Почему thresholds для timing-тестов должны быть щедрыми?
  - Как Profiler помогает тестировать мемоизацию React-компонентов?
  - Что лучше использовать для отправки метрик при уходе со страницы?
  - Какой антипаттерн возникает при установке бюджета performance score = 1.0?
---

# Тестирование производительности

Быстрое приложение снижает отказы и улучшает конверсию. В этой статье разберём, как автоматизировать проверки производительности фронтенда и API: от Lighthouse CI и Web Vitals до нагрузочных тестов и мониторинга реальных пользователей. Вы научитесь задавать performance budgets, профилировать компоненты React и Vue, а также ловить регрессии до попадания кода в production.
...
8. [Профилирование Vue-компонентов](#профилирование-vue-компонентов)
9. [Тестирование времени выполнения](#тестирование-времени-выполнения)
10. [Мониторинг производительности в production](#мониторинг-производительности-в-production)
...
- Тестирование производительности охватывает загрузку страницы, размер бандла, отзывчивость интерфейса и нагрузку на API.
- Lighthouse CI автоматизирует запуск Lighthouse в CI/CD и позволяет задавать пороги для категорий и отдельных метрик.
- Core Web Vitals измеряют LCP, INP и CLS, а в headless-тестах их стоит использовать как baseline, а не абсолютные значения.
- size-limit проверяет реальный размер бандла после tree-shaking и gzip/brotli, предотвращая регрессии в CI.
- k6 моделирует виртуальных пользователей, а stages и thresholds помогают воспроизвести реалистичную нагрузку и автоматически провалить тест.
- Performance budgets фиксируют лимиты на метрики и размеры чанков, чтобы регрессии останавливались ещё на этапе PR.
- React Profiler в тестах выявляет грубые регрессии рендера, но добавляет overhead и не заменяет production-профилирование.
- Timing-тесты проверяют, что функции укладываются в разумные сроки, и должны использовать щедрые пороги, чтобы не быть flaky.
- Production-мониторинг через web-vitals собирает реальные метрики пользователей, которые часто отличаются от синтетических лабораторных данных.
- Антипаттерны включают игнорирование производительности, нереалистичные бюджеты, тестирование только в идеальных условиях и оптимизацию без измерений.

## Заключение

1. Производительность нужно проверять автоматически на каждом этапе: от локальной сборки до production-мониторинга.
2. Lighthouse CI, Web Vitals, size-limit и k6 покрывают основные риски регрессий скорости фронтенда и API.
3. Профилирование React и Vue помогает находить медленные компоненты, но его результаты в тестах следует интерпретировать как baseline.
4. Performance budgets и щедрые thresholds делают проверки стабильными и предсказуемыми в CI.
5. Реальные пользователи работают в условиях, отличных от лаборатории, поэтому RUM обязателен для полной картины.
6. Измеряйте до оптимизации, иначе усилия уйдут в неправильном направлении.

## Полезные ссылки

- [Lighthouse CI](https://github.com/GoogleChrome/lighthouse-ci) — автоматизация Lighthouse в CI/CD.
- [web-vitals](https://github.com/GoogleChrome/web-vitals) — библиотека Google для сбора метрик в браузере.
- [size-limit](https://github.com/ai/size-limit) — контроль размера JavaScript-бандла.
- [k6](https://k6.io/) — нагрузочное тестирование API от Grafana.
- [Artillery](https://www.artillery.io/) — YAML-конфигурируемый инструмент нагрузочных тестов.
- [Core Web Vitals](https://web.dev/articles/vitals) — официальное руководство Google по ключевым метрикам.
- [MDN — PerformanceObserver](https://developer.mozilla.org/en-US/docs/Web/API/PerformanceObserver) — API для сбора производительности в браузере.
- [MDN — navigator.sendBeacon](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/sendBeacon) — надёжная отправка аналитики при закрытии страницы.
