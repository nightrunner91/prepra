...
questions:
  - "Чем визуальное тестирование отличается от обычных юнит-тестов"
  - "Какие виды визуального тестирования существуют и когда их применять"
  - "Как работает snapshot-тестирование и какие у него ограничения"
  - "Как Playwright сравнивает скриншоты и зачем нужны mask, threshold и maxDiffPixelRatio"
  - "Чем Chromatic отличается от Percy и когда выбрать каждый сервис"
  - "Как Storybook помогает визуальному тестированию компонентов"
  - "Как тестировать адаптивность и переключение тем"
  - "Почему анимации делают визуальные тесты flaky и как с ними бороться"
  - "Зачем ждать document.fonts.ready перед скриншотами"
  - "Как правильно хранить и обновлять baseline-скриншоты в CI/CD"
---
...
- Визуальное тестирование проверяет внешний вид компонентов, а не только логику.
- Snapshot-тесты фиксируют DOM-структуру и требуют ручного ревью перед обновлением.
- Визуальная регрессия сравнивает пиксели скриншотов и ловит непреднамеренные изменения дизайна.
- Playwright предоставляет `toHaveScreenshot()` для локального скриншотного тестирования.
- Маскирование динамического контента необходимо для стабильности визуальных тестов.
- Анимации следует отключать, иначе скриншоты станут flaky.
- Chromatic интегрируется со Storybook и делает скриншоты stories в облаке.
- Percy поддерживает скриншоты произвольных страниц и разные браузеры.
- Storybook изолирует компоненты и упрощает визуальное тестирование состояний.
- Тестирование на разных viewports и темах помогает ловить адаптивные баги.
- Baseline-скриншоты коммитят в репозиторий, а в CI только сравнивают, не обновляют.
- Для шрифтов нужно дождаться `document.fonts.ready` перед скриншотом.
