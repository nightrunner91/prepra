...
questions:
  ...
  - "Чем snapshot-тесты отличаются от визуальной регрессии и когда какой подход применять"
  - "Что произойдёт, если не маскировать динамический контент и не отключать анимации перед скриншотами"
  - "Как Storybook, Chromatic и Percy решают проблему визуального тестирования на разных уровнях"
  - "Почему baseline-скриншоты нужно коммитить в репозиторий и как это влияет на CI/CD"
  - "Какие антипаттерны делают визуальные тесты нестабильными и как их избежать"
---

# Визуальное и регрессионное тестирование

Визуальные баги часто ускользают от обычных юнит-тестов: компонент может работать правильно, но выглядеть сломанным. В статье разбираем инструменты и подходы, которые позволяют автоматически отслеживать изменения внешнего вида. Вы научитесь настраивать snapshot-тесты, скриншотное сравнение в Playwright, облачные сервисы и интеграцию в CI/CD.
...
- Визуальное тестирование проверяет внешний вид компонентов, который не ловят функциональные тесты; snapshot-тесты фиксируют DOM-структуру, а визуальная регрессия сравнивает пиксели скриншотов для обнаружения непреднамеренных изменений дизайна.
- Snapshot-тесты подходят для стабильной DOM-структуры, но требуют ручного ревью перед обновлением; визуальная регрессия (Playwright `toHaveScreenshot()`) ловит реальные рендеринг-баги, но чувствительна к динамическому контенту и анимациям.
- Маскирование динамического контента (даты, аватары) и отключение анимаций необходимы для стабильности; ожидание `document.fonts.ready` предотвращает flaky-тесты из-за незагруженных шрифтов.
- Storybook изолирует компоненты для тестирования состояний; Chromatic интегрируется со Storybook для облачного скриншотного ревью; Percy поддерживает произвольные страницы и кросс-браузерность.
- Тестирование на разных viewports и темах ловит адаптивные баги; baseline-скриншоты коммитят в репозиторий, а в CI только сравнивают, не обновляют.
- Антипаттерны: snapshot всего подряд, игнорирование масок для динамики, обновление baseline без ревью, тестирование только в идеальных условиях.

---

## Заключение

Визуальное тестирование — это страховка от незаметных изменений внешнего вида. Snapshot-тесты подходят для стабильной DOM-структуры, а пиксельное сравнение скриншотов — для проверки реального рендеринга. Инструменты вроде Playwright, Chromatic и Percy закрывают разные сценарии: локальные тесты, облачное ревью и кросс-браузерность. Чтобы тесты были стабильны, маскируйте динамику, отключайте анимации и тестируйте ключевые viewports. Регулярно прогоняйте визуальные тесты в CI и ревьюйте diff перед обновлением baseline.
