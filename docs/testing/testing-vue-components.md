...
questions:
  ...
  - "Как монтировать компонент с зависимостями (Pinia, Vue Router, provide) и зачем нужна опция global"
  - "Что произойдёт, если не дождаться flushPromises в асинхронных тестах, и как это влияет на DOM"
  - "Как тестировать v-model через связку modelValue и update:modelValue и почему это важнее проверки внутреннего состояния"
  - "Какие антипаттерны (сложные CSS-селекторы, зависимость тестов, snapshot всего) делают тесты Vue хрупкими и как их избежать"
---
...
- Vue Test Utils предоставляет `wrapper` с доступом к DOM и инстансу, но предпочтительнее проверять поведение пользователя, а не `wrapper.vm`; `mount` создаёт изолированный инстанс, в `global` передаются плагины, provide, stubs и моки.
- Для поиска элементов используют `find`, `findAll`, `findComponent`; семантические селекторы надёжнее CSS-классов, а `data-testid` — последний резерв; `trigger` имитирует DOM-события, `setValue` упрощает работу с полями ввода.
- Пропсы проверяются через рендеринг, классы и текст, `setProps` позволяет проверить реакцию на изменения; события проверяют через `wrapper.emitted()`, а `v-model` тестируют через связку `modelValue` и `update:modelValue`.
- Слоты передаются через опцию `slots` (именованные и scoped); асинхронные тесты требуют `flushPromises` или `vi.waitFor`, чтобы дождаться обновления DOM.
- Pinia тестируют с `createPinia`/`createTestingPinia`, Vue Router — через создание роутера и передачу в `global.plugins`; `provide/inject` задаётся в `global.provide`, повторяющиеся зависимости выносят в кастомную функцию `mount`.
- Антипаттерны: завязка на внутреннее состояние, сложные CSS-селекторы, зависимые друг от друга тесты, игнорирование асинхронности, избыточное мокирование, snapshot всего компонента и тесты без assertions.
