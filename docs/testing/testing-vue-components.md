...
questions:
  - "Чем философия Vue Test Utils отличается от React Testing Library"
  - "Как монтировать компонент и передавать пропсы через mount"
  - "Как найти элемент и имитировать клик или ввод текста"
  - "Как проверить, что компонент эмитнул событие и с какими аргументами"
  - "Как протестировать именованные и scoped-слоты"
  - "Как связать v-model в тестах через modelValue и update:modelValue"
  - "Как дождаться завершения асинхронной операции в компоненте"
  - "Как подключить Pinia или Vue Router при монтировании компонента"
  - "Как избежать тестирования внутреннего состояния компонента"
  - "Когда стоит использовать data-testid"
---

# Тестирование Vue-компонентов с Vue Test Utils

Тестирование компонентов позволяет уверенно рефакторить интерфейс и выявлять регрессии до попадания кода в прод. В статье разбираем Vue Test Utils — официальную библиотеку для монтирования и взаимодействия с Vue-компонентами в изолированном окружении. Вы научитесь настраивать Vitest, писать тесты пропсов, событий, слотов, асинхронной логики, Pinia и Vue Router, а также избегать типичных хрупких проверок.
...
- Vue Test Utils предоставляет `wrapper` с доступом к DOM, инстансу и событиям, но предпочтительнее проверять поведение пользователя, а не `wrapper.vm`.
- `mount` создаёт изолированный инстанс компонента; в `global` передаются плагины, provide, stubs и моки.
- Для поиска элементов используют `find`, `findAll`, `findComponent`; семантические селекторы надёжнее CSS-классов, а `data-testid` — последний резерв.
- `trigger` имитирует DOM-события и возвращает Promise; `setValue` упрощает работу с полями ввода.
- Пропсы проверяются через рендеринг, классы и текст; `setProps` позволяет проверить реакцию на изменения.
- События проверяют через `wrapper.emitted()`, а `v-model` тестируют через связку `modelValue` и `update:modelValue`.
- Слоты передаются через опцию `slots`, включая именованные и scoped-слоты.
- Асинхронные тесты требуют `flushPromises` или `vi.waitFor`, чтобы дождаться обновления DOM.
- Pinia тестируют с `createPinia`/`createTestingPinia`, Vue Router — через создание роутера и передачу в `global.plugins`.
- `provide/inject` задаётся в `global.provide`; часто повторяющиеся зависимости выносят в кастомную функцию `mount`.
- Лучшие практики: тестировать поведение, очищать DOM и моки, мокировать внешние API через MSW и избегать snapshot всего компонента.
- Антипаттерны: завязка на внутреннее состояние, сложные CSS-селекторы, зависимые друг от друга тесты, игнорирование асинхронности, избыточное мокирование и тесты без assertions.

## Заключение

Vue Test Utils даёт удобный API для компонентного тестирования, но его мощь — доступ к инстансу — легко превращается в хрупкие тесты. Основная цель: проверять то, что видит пользователь, — текст, кнопки, формы и эмитированные события. Для сложных сценариев используйте `flushPromises`, кастомные `mount`-обёртки и `createTestingPinia`. Настройте `cleanup` и `clearAllMocks` в `afterEach`, мокируйте API через MSW и выбирайте семантические селекторы. Регулярная практика поможет писать устойчивые тесты, которые не ломаются при рефакторинге.
