...
questions:
  ...
  - "Как TypeScript выводит тип дженерика из контекста (type inference)"
  - "Что такое type erasure и почему дженерики существуют только на этапе компиляции"
  - "Как работают constraints (`extends`) и зачем они нужны"
  - "Что делает `keyof T` и как используется `T[K]`"
  - "Как работают типы по умолчанию в дженериках"
  - "Как типизировать generic-компоненты в React и Vue"
  - "Как читать сложные дженерики по шагам"
---

# Дженерики TypeScript: constraints и паттерны

Дженерики — одна из самых мощных и одновременно самых сложных концепций TypeScript. Они позволяют писать код, который работает с любыми типами, сохраняя при этом полную типобезопасность.

Дженерик — это «переменная для типа». Как обычная функция принимает значение и возвращает результат, так дженерик принимает тип и возвращает типизированный результат. В React дженерики используются в хуках, компонентах, utility-типах. Во Vue — в composables, пропсах, хелперах. В этой статье разберём дженерики от базовых концепций до продвинутых паттернов, применяемых в реальных проектах.
...
- Дженерики — «переменные для типов»: позволяют писать переиспользуемый код без потери типобезопасности.
- TypeScript выводит тип дженерика из контекста (type inference), но иногда нужно указывать явно.
- Дженерики существуют только на этапе компиляции — в рантайме происходит type erasure.
- `extends` ограничивает, какие типы можно передать в дженерик: `T extends HasId` требует наличие `id`.
- `keyof T` возвращает union всех ключей; `T[K]` — тип значения для ключа `K`.
- Типы по умолчанию работают справа налево: обязательные параметры идут первыми.
- В React дженерики используются в `useState<T>`, `useReducer`, generic-компонентах, `forwardRef`.
- Во Vue дженерики применяются в `defineProps<T>`, `defineEmits`, composables с `generic="T"`.
- Описательные имена (`TData`, `TError`) улучшают читаемость при трёх и более параметрах типа.
- Встроенные utility-типы (`Partial`, `Pick`, `Omit`, `Record`) — это дженерики, трансформирующие типы.
...
- [Generic Types — TypeScript Deep Dive](https://basarat.gitbook.io/typescript/type-system/generics)
