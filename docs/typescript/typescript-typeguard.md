...
TypeScript добавляет статическую типизацию, но не меняет поведение рантайма. Это создаёт разрыв: типы существуют только при компиляции, а значения — только при выполнении. TypeGuard — механизм, который сокращает этот разрыв, позволяя компилятору сужать типы на основе runtime-проверок. В этой статье разберём control flow analysis, type predicates, assertion functions и научимся валидировать данные из внешних источников.

## Содержание

1. [Фундаментальная проблема](#фундаментальная-проблема)
2. [Control Flow Analysis](#control-flow-analysis)
3. [Ограничения встроенных сужений](#ограничения-встроенных-сужений)
4. [Type Predicates — пользовательские type guard](#type-predicates--пользовательские-type-guard)
5. [Assertion Functions](#assertion-functions)
6. [Практическое применение](#практическое-применение)
7. [Когда type guard недостаточен](#когда-type-guard-недостаточен)
8. [Ключевые принципы](#ключевые-принципы)
...
- Type guard — мост между рантаймом и системой типов: позволяет runtime-проверкам влиять на compile-time анализ.
- Control flow analysis — основа сужения типов: TypeScript отслеживает типы в каждой точке программы и сужает их при ветвлении.
- `typeof` работает только с примитивами и `function`; для объектов возвращает `"object"` без информации о структуре.
- `instanceof` проверяет цепочку прототипов; `in` проверяет наличие свойства — оба сужают тип.
- Discriminated unions с общим полем-литералом сужаются через `switch` или `if` без дополнительных type guard.
- Предикат типа (`value is T`) — контракт, а не проверка: TypeScript доверяет вам, корректность на вашей совести.
- Assertion functions (`asserts value is T`) — альтернатива для сценариев, где неудача бросает исключение.
- Type guard работает как callback для `.filter()`, `.find()` — TypeScript корректно сузит тип элементов.
- Exhaustiveness check с `never` в `default`-ветке гарантирует обработку всех вариантов union.
- Для сложной валидации используйте Zod, Valibot — они одновременно проверяют данные и генерируют типы.

## Заключение

Type guard — необходимый инструмент для работы с данными извне: API, парсинг, пользовательский ввод. Встроенные механизмы (`typeof`, `instanceof`, `in`, discriminated unions) покрывают простые случаи. Для сложной логики используйте предикаты типа (`value is T`) и assertion functions (`asserts value is T`). Помните: предикат — это контракт, а не проверка. Для серьёзной валидации предпочитайте Zod или аналоги — они объединяют runtime-проверку с генерацией типов.

## Полезные ссылки

- [Narrowing — TypeScript Documentation](https://www.typescriptlang.org/docs/handbook/2/narrowing.html)
- [Type Predicates — TypeScript Documentation](https://www.typescriptlang.org/docs/handbook/2/narrowing.html#using-type-predicates)
