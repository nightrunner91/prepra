---
title: "TypeScript `infer` — выводи типы как детектив"
section: typescript
description: "`infer` — это ключевое слово TypeScript, которое позволяет **выводить типы внутри условных типов**. Звучит страшно, но на деле — это как детектив: ты смотришь на структуру типа и говоришь \"ага, вот..."
order: 6
tags: ["typescript", "infer", "выводи", "типы", "детектив"]
questions:
  - "Какую проблему решает `infer` и где он используется"
  - "Как читать конструкции с `infer` по шагам"
  - "Как извлечь возвращаемый тип функции через `infer`"
  - "Как извлечь тип элемента массива через `infer`"
  - "Как распаковать вложенный Promise с рекурсией"
...
`infer` — это ключевое слово TypeScript, которое позволяет **выводить типы внутри условных типов**. Звучит страшно, но на деле — это как детектив: ты смотришь на структуру типа и говоришь "ага, вот тут лежит число, а тут — строка".

`infer` незаменим при создании utility-типов: он позволяет «залезть» внутрь типа и извлечь нужную информацию — возвращаемый тип функции, элемент массива, значение Promise. В этой статье разберём `infer` с самых основ, с аналогиями и пошаговыми примерами — от базовых паттернов до реального применения в React.
...
- `infer` позволяет выводить типы внутри условных типов — «залезать» внутрь типа и извлекать информацию.
- Синтаксис: `T extends SomePattern<infer X> ? X : never` — «если T подходит под шаблон, назовём содержимое X и вернём X».
- `infer` работает только в `extends`-ветке условного типа и только в той ветке, где объявлен.
- Извлечение возвращаемого типа функции: `T extends (...args: any[]) => infer R ? R : never`.
- Извлечение элемента массива: `T extends (infer E)[] ? E : never`.
- Распаковка Promise с рекурсией: `T extends Promise<infer U> ? Awaited<U> : T`.
- Несколько `infer` одновременно: `T extends [infer First, ...infer Rest] ? Rest : never`.
- TS 4.7+ позволяет накладывать ограничения на `infer`: `infer U extends number | string`.
- Нельзя использовать одно имя `infer` дважды в одном шаблоне.
- `infer` в React: извлечение props из компонента, типа из хука, ref из RefObject.

## Полезные ссылки

- [infer — TypeScript Documentation](https://www.typescriptlang.org/docs/handbook/2/conditional-types.html)
- [Conditional Types — TypeScript Deep Dive](https://basarat.gitbook.io/typescript/type-system/conditional-types)
- [TypeScript infer — Ultimate TypeScript Guide](https://www.totaltypescript.com/tutorials/typescript/infer)
