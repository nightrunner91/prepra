...
`useContext` — хук для чтения данных из Context, минуя промежуточные уровни компонентов. Вместе с `createContext` и `Provider` он решает проблему prop drilling — передачи данных через компоненты, которым они не нужны. В React 19 `use(context)` заменяет `useContext` и добавляет возможность условного вызова.
...
- `useContext` читает данные из ближайшего `<Provider>` выше в дереве, минуя промежуточные компоненты.
- Решает проблему prop drilling — передачи данных через компоненты, которым они не нужны.
- При изменении `value` в Provider все потребители перерендериваются — нужна оптимизация.
- Разделение state/dispatch на два контекста предотвращает лишние ререндеры.
- `useMemo` для значения контекста предотвращает пересоздание объекта при каждом рендере.
- `useReducer + useContext` — минималистичная альтернатива Redux без зависимостей.
- Context идеален для редко изменяющихся данных: тема, локаль, авторизация.
- Для высокочастотных обновлений используйте Zustand с селекторами.
- В React 19 `use(context)` заменяет `useContext` и добавляет условный вызов.

## Заключение

`useContext` — механизм передачи данных через дерево компонентов без prop drilling. Он решает конкретную проблему, но не заменяет библиотеки управления состоянием. Context идеален для темы, локали, авторизации — данных, которые меняются редко. Для высокочастотных обновлений используйте Zustand с селекторами. Связка `useReducer + useContext` — минималистичная альтернатива Redux. В React 19 `use(context)` заменяет `useContext` и добавляет возможность условного вызова.
