...
questions:
  ...
  - "Как работает базовая механика HOC: функция принимает компонент и возвращает новый"
  - "Какие проблемы возникают с HOC: потеря статических методов, конфликт пропсов, DevTools"
  - "Почему порядок композиции HOC влияет на поведение"
  - "Чем HOC отличается от хуков и почему хуки вытеснили HOC"
  - "Какие встроенные HOC React остаются без альтернатив в виде хуков"
  - "Почему HOC внутри рендера — антипаттерн"
---

# HOC: паттерн обёрток, композиция и сравнение с хуками
...
- HOC — функция, принимающая компонент и возвращающая новый компонент с расширенным поведением.
- HOC не мутирует исходный компонент, а создаёт обёртку; пропсы передаются через.
- `displayName` обязателен для читаемого дерева в DevTools.
- Композиция HOC через `compose` инвертирует порядок вложенности для читаемости.
- Порядок композиции влияет на поведение: `withAuth(withData(...))` ≠ `withData(withAuth(...))`.
- Хуки вытеснили HOC для переиспользования логики: прозрачность данных, плоская композиция, лучшая типизация.
- Встроенные HOC React (`memo`, `forwardRef`, `lazy`) остаются без альтернатив в виде хуков.
- HOC внутри рендера — антипаттерн: приводит к размонтированию и потере состояния.

## Заключение

Higher-Order Components — важный паттерн в истории React, решавший задачу переиспользования логики до появления хуков. В 2026 году HOC занял свою нишу: встроенные `React.memo`, `React.forwardRef`, `React.lazy` для мемоизации и ленивой загрузки, Error Boundary и Suspense-обёртки. Для переиспользования логики с состоянием используйте хуки. В legacy-коде HOC встречается часто — понимание паттерна обязательно для чтения чужого кода.

## Полезные ссылки

- [Higher-Order Components](https://react.dev/reference/react/forwardRef)
