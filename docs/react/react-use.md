...
- `use()` разворачивает Promise или Context прямо в теле компонента на этапе рендера.
- Единственный хук React, который можно вызывать условно (в `if`, циклах, после `return`).
- Pending Promise приостанавливает компонент — ближайший `<Suspense>` показывает fallback.
- Rejected Promise автоматически ловится ближайшей `<ErrorBoundary>`.
- Promise должен быть стабильным между рендерами — иначе бесконечный цикл приостановок.
- `use(context)` заменяет `useContext(context)` с возможностью условного вызова.
- Render-as-you-fetch: промис создаётся до рендера, что устраняет waterfall.
- Трёхслойная архитектура: ErrorBoundary → Suspense → Компонент.
- На серверных компонентах используйте `async/await`, а не `use()`.

## Заключение

Хук `use()` — рекомендованный способ чтения Promise и Context в React 19. Он устраняет бойлерплейт из `useState`/`useEffect`, предотвращает waterfall-запросы через render-as-you-fetch и интегрируется с Suspense и Error Boundaries. Ключевые правила: стабильный Promise, наличие `<Suspense>` в дереве, трёхслойная обёртка с ErrorBoundary. Для серверных компонентов используйте `async/await`.
