...
1. [Приоритеты обновлений в Concurrent Mode](#приоритеты-обновлений-в-concurrent-mode)
2. [useTransition — явная маркировка несрочных обновлений](#usetransition--явная-маркировка-несрочных-обновлений)
3. [useDeferredValue — отложенное значение для неконтролируемых обновлений](#usedeferredvalue--отложенное-значение-для-неконтролируемых-обновлений)
4. [Практические паттерны и рекомендации](#практические-паттерны-и-рекомендации)
5. [Производительность и отладка](#производительность-и-отладка)
6. [Сравнение с Vue](#сравнение-с-vue)
...
- Concurrent Mode позволяет React прерывать и возобновлять рендеринг, обрабатывая обновления по приоритету.
- `useTransition` понижает приоритет обновления состояния, возвращает `isPending` для индикации загрузки.
- `useDeferredValue` откладывает реакцию на значение из пропсов, когда нет доступа к сеттеру.
- `startTransition` оборачивает несрочные обновления; синхронные обновления (клики) прерывают transition.
- `useTransition` — не CSS-анимация, это термин конкурентного режима React.
- Для drag-and-drop и позиционирования курсора transition не подходит — нужны синхронные обновления.
- Конкурентный режим не ускоряет вычисления, а перераспределяет их для отзывчивости UI.
- Комбинация с `React.memo` и `useMemo` усиливает эффект от приоритизации.

## Заключение

`useTransition` и `useDeferredValue` — мощные инструменты для создания отзывчивых интерфейсов в React 18+. Используйте `useTransition`, когда контролируете сеттер состояния, и `useDeferredValue`, когда значение приходит извне. Помечайте как transition только несрочные обновления и показывайте индикаторы загрузки через `isPending` или сравнение значений. Конкурентный режим — не серебряная пуля: для действительно тяжёлых задач используйте Web Workers, виртуализацию и другие оптимизации.

## Полезные ссылки

- [useTransition](https://react.dev/reference/react/useTransition)
- [useDeferredValue](https://react.dev/reference/react/useDeferredValue)
- [Concurrency in React](https://react.dev/blog/2022/03/29/react-v18#concurrent-features)
