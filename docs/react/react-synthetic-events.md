...
questions:
  ...
  - "Как работает делегирование событий в React"
  - "Что изменилось в React 17 с точкой делегирования"
  - "Почему пул событий удалён в React 17 и что это значит"
  - "Как получить доступ к нативному событию через e.nativeEvent"
  - "Какие приоритеты назначаются событиям в Concurrent Mode"
  - "Как события работают с React Portals"
  - "Чем обработка событий в React отличается от Vue"
---

# Синтетические события в React

Синтетические события (Synthetic Events) — это кросс-браузерная обёртка React над нативными DOM-событиями. Они обеспечивают единый API для обработки событий, абстрагируя разработчика от различий между браузерами и оптимизируя производительность через делегирование. В этой статье разберём механику работы, эволюцию делегирования, изменения в React 17+ и практические паттерны.

## Содержание

1. [Что такое синтетические события](#что-такое-синтетические-события)
2. [Как работает система событий React](#как-работает-система-событий-react)
3. [Зачем нужны синтетические события](#зачем-нужны-синтетические-события)
4. [Изменения в React 17+](#изменения-в-react-17)
5. [Доступ к нативным событиям](#доступ-к-нативным-событиям)
6. [Синтетические события в Concurrent Mode](#синтетические-события-в-concurrent-mode)
7. [Типы поддерживаемых событий](#типы-поддерживаемых-событий)
8. [Практические примеры](#практические-примеры)
9. [Когда синтетические события могут быть проблемой](#когда-синтетические-события-могут-быть-проблемой)
10. [Отличия от Vue](#отличия-от-vue)
...
- SyntheticEvent — кросс-браузерная обёртка над нативным событием с единым API.
- React использует делегирование: один listener на root-контейнере вместо тысяч на элементах.
- С React 17 делегирование на `root`, а не на `document` — решает проблемы микрофронтендов.
- Пул событий удалён в React 17: SyntheticEvent можно использовать асинхронно без `persist()`.
- `e.nativeEvent` даёт доступ к оригинальному браузерному событию.
- События имеют приоритеты: click/keydown — Discrete, input — Continuous, pointerMove — Default.
- Portals: события всплывают через React-дерево, а не через DOM-дерево.
- Во Vue нет SyntheticEvent — используются нативные события с модификаторами (`.stop`, `.prevent`).

## Заключение

Синтетические события — основа системы обработки событий в React. Они обеспечивают кросс-браузерную совместимость, производительность через делегирование и интеграцию с батчингом обновлений. В React 17+ система стала проще (удалён пул событий) и эффективнее (делегирование на root). Для повседневной разработки они работают прозрачно, но понимание механики помогает при интеграции со сторонними библиотеками и работе с Concurrent Mode.

## Полезные ссылки

- [React Events](https://react.dev/reference/react-dom/components/common#react-event-handler)
