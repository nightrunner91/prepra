...
questions:
  ...
  - "Почему хуки вызываются строго в порядке написания"
  - "Чем Render Phase отличается от Commit Phase"
  - "Почему эффекты вызываются снизу вверх, а рендер — сверху вниз"
  - "Что делает StrictMode и почему он вызывает эффекты дважды"
  - "Когда использовать useLayoutEffect вместо useEffect"
  - "Как React батчит несколько setState в один ререндер"
---
...
1. [Две фазы рендеринга](#две-фазы-рендеринга)
2. [Порядок рендеринга компонентов](#порядок-рендеринга-компонентов)
3. [Порядок вызова хуков](#порядок-вызова-хуков)
4. [Правила хуков (Rules of Hooks)](#правила-хуков-rules-of-hooks)
5. [Порядок вызова эффектов (useEffect / useLayoutEffect)](#порядок-вызова-эффектов-useeffect--uselayouteffect)
6. [Полный порядок при монтировании](#полный-порядок-при-монтировании)
7. [Полный порядок при обновлении (re-render)](#полный-порядок-при-обновлении-re-render)
8. [Полный порядок при размонтировании](#полный-порядок-при-размонтировании)
9. [Strict Mode и двойной вызов](#strict-mode-и-двойной-вызов)
10. [Практические следствия](#практические-следствия)
...
- Render Phase (сверху вниз) — прерываемая, чистая, без побочных эффектов. Commit Phase — непрерываемая, синхронная.
- Компоненты рендерятся сверху вниз: родитель → дети, в порядке объявления в JSX.
- Хуки вызываются строго в порядке написания; порядок фиксирован между рендерами.
- Эффекты (`useLayoutEffect`, `useEffect`) вызываются снизу вверх: дети → родители.
- `useLayoutEffect` — синхронно до paint; `useEffect` — асинхронно после paint.
- Cleanup-функции вызываются снизу вверх при размонтировании или перед повторным запуском эффекта.
- StrictMode вызывает компоненты и эффекты дважды в dev-режиме для обнаружения побочных эффектов.
- React батчит несколько `setState` в один ререндер.
- Условный вызов хуков нарушает правила и приводит к багам.

## Заключение

Порядок рендеринга в React подчиняется строгой логике: компоненты рендерятся сверху вниз, хуки вызываются по порядку написания, эффекты — снизу вверх. Понимание этих правил необходимо для написания предсказуемого кода, корректной работы с DOM-измерениями и избежания race conditions. Всегда начинайте с `useEffect`; переходите на `useLayoutEffect` только при визуальном моргании.
