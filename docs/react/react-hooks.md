...
questions:
  ...
  - "Как правильно обновлять объекты и массивы в useState иммутабельно"
  - "Что такое функциональное обновление setState и зачем оно нужно"
  - "Когда useReducer предпочтительнее useState"
  - "Какие правила хуков существуют и почему порядок вызова важен"
  - "Чем useMemo отличается от useCallback"
  - "Как работают пользовательские хуки и какие правила их разработки"
  - "Что делает use() и почему его можно вызывать условно"
---

# Хуки React: useState, useEffect, useReducer и пользовательские хуки
...
1. [useRef — храним значение между рендерами без перерисовки](#useref--храним-значение-между-рендерами-без-перерисовки)
2. [useState — фундамент локального состояния](#usestate--фундамент-локального-состояния)
3. [useReducer — управление сложным состоянием](#usereducer--управление-сложным-состоянием)
4. [useEffect — побочные эффекты и их очистка](#useeffect--побочные-эффекты-и-их-очистка)
5. [useMemo и useCallback — мемоизация значений и функций](#usememo-и-usecallback--мемоизация-значений-и-функций)
6. [Пользовательские хуки — переиспользуемая логика](#пользовательские-хуки--переиспользуемая-логика)
7. [useTransition — неблокирующие обновления](#usetransition--неблокирующие-обновления)
8. [useDeferredValue — отложенное значение](#usedeferredvalue--отложенное-значение)
9. [Общие правила хуков](#общие-правила-хуков)
...
- `useRef` хранит значение между рендерами без перерисовки; изменение `.current` не вызывает ре-рендер.
- `useState` обновляет состояние иммутабельно; функциональное обновление `prev => ...` предотвращает stale closure.
- `useReducer` — `(state, action) => nextState`; атомарные обновления, тестируемость без React, `dispatch` стабилен.
- `useEffect` выполняется после commit; честные зависимости обязательны; cleanup-функция чистит подписки и таймеры.
- `useMemo` кэширует значение, `useCallback` — ссылку на функцию; React Compiler автоматизирует мемоизацию.
- Пользовательские хуки начинаются с `use`, хранят одну ответственность, вызываются на верхнем уровне.
- `useTransition` понижает приоритет обновления; `useDeferredValue` откладывает реакцию на значение из пропсов.
- Правила хуков: только на верхнем уровне, только из компонентов/хуков, порядок фиксирован.
- `use()` (React 19) — единственный хук, который можно вызывать условно.

## Заключение

Хуки — не просто синтаксический сахар, а переосмысление того, как в React моделируется логика с состоянием. Их главная сила в композиции: маленькие хуки комбинируются в пользовательские, создавая переиспользуемый код. Конкурентный рендеринг добавил `useTransition` и `useDeferredValue` для плавных интерфейсов. React Compiler снимает бремя ручной мемоизации. Знание хуков и их правил — обязательное требование для React-разработчика.

## Полезные ссылки

- [Hooks API Reference](https://react.dev/reference/react/hooks)
- [Rules of Hooks](https://react.dev/reference/rules/rules-of-hooks)
- [Hooks FAQ](https://react.dev/reference/rules/rules-of-hooks)
