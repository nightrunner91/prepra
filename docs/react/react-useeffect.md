...
questions:
  ...
  - "Как работает массив зависимостей и Object.is для сравнения"
  - "Что такое stale closure и как его избежать"
  - "Зачем нужна cleanup-функция и что обязательно чистить"
  - "Почему нельзя вызывать async-функцию как колбэк useEffect напрямую"
  - "Как StrictMode вызывает эффект дважды и зачем"
  - "Почему производные данные лучше вычислять в рендере, а не в useEffect"
  - "Чем watchEffect из Vue отличается от useEffect"
---

# useEffect: побочные эффекты, зависимости и cleanup

`useEffect` — хук для выполнения побочных эффектов в функциональных компонентах React: запросы к API, подписки на события, работа с DOM, таймеры, интеграция со сторонними библиотеками. Эффект выполняется после того, как React закоммитил изменения в DOM. В этой статье разберём механику работы, массив зависимостей, cleanup-функцию, сравнение с Vue и типичные ошибки.
...
- `useEffect` выполняется после commit phase — изменения уже в DOM.
- Массив зависимостей определяет, когда эффект перезапускается; `Object.is` для сравнения.
- Честные зависимости обязательны — иначе stale closure (устаревшее замыкание).
- Cleanup-функция чистит подписки, таймеры, AbortController перед повторным запуском и при размонтировании.
- `useLayoutEffect` — синхронно до paint, для измерений DOM; `useEffect` — асинхронно после paint.
- StrictMode вызывает эффект дважды в dev для обнаружения проблем с очисткой.
- Каждый эффект — одна ответственность; не смешивайте несвязанную логику.
- Производные данные вычисляйте в рендере (или через `useMemo`), а не в `useEffect`.
- В React 19 для загрузки данных предпочтительнее `use()` + `<Suspense>`.

## Заключение

`useEffect` — основной хук для побочных эффектов в React. Ключевые правила: честные зависимости, обязательная очистка подписок и таймеров, один эффект — одна ответственность. Всегда начинайте с `useEffect`; переходите на `useLayoutEffect` только при визуальном моргании. Для загрузки данных в React 19 предпочтительнее `use()` + `<Suspense>`, а не `useEffect` + `useState`.
