---
title: "Suspense: декларативная загрузка, потоковый SSR и хук use()"
section: react
description: "Компонент React для декларативного управления состоянием загрузки. Механика throw-promise, вложенные границы, потоковый SSR, интеграция с Error Boundaries и эволюция от React 16 до 19.2."
order: 9
tags: ["suspense", "lazy", "streaming-ssr", "use", "error-boundary"]
questions:
  - "Какую проблему решает Suspense по сравнению с ручным управлением loading/error"
  - "Что такое механизм throw-promise и как он работает"
  - "Зачем нужны вложенные границы Suspense"
  - "Как Suspense устраняет waterfall-запросы через render-as-you-fetch"
  - "Почему ErrorBoundary должен быть снаружи, а Suspense внутри"
  - "Как работает потоковый SSR с Suspense"
  - "Что изменилось в React 19.2 с батчированием границ Suspense"
  - "Какие антипаттерны связаны с Suspense"
---

# Suspense: декларативная загрузка, потоковый SSR и хук use()

`<Suspense>` — компонент React для декларативного управления состоянием загрузки. Вместо того чтобы каждый компонент вручную обрабатывал `loading`/`error`/`success`, Suspense выносит эту обязанность на инфраструктурный уровень. В сочетании с хуком `use()`, потоковым SSR и конкурентным рендерингом он формирует основу современной архитектуры загрузки данных.
...
- Suspense позволяет декларативно описывать состояние загрузки через `fallback` для целого поддерева.
- Механика: компонент «выбрасывает» промис, React ловит его в ближайшей границе Suspense.
- Вложенные границы Suspense обеспечивают прогрессивную загрузку независимых секций.
- Render-as-you-fetch: промис создаётся до рендера, что устраняет waterfall-запросы.
- `use()` (React 19) — основной способ разворачивания Promise в компоненте.
- ErrorBoundary снаружи, Suspense внутри — правильный порядок для обработки ошибок.
- Потоковый SSR: HTML отправляется прогрессивно по мере готовности данных.
- React 19.2 батчирует границы Suspense в SSR для атомарного появления контента.
- `useTransition` сохраняет старый UI видимым при навигации, пока загружается новый.

## Заключение

Suspense — фундаментальный переход от императивного управления асинхронными состояниями к декларативному. Он позволяет описывать загрузку один раз для поддерева, прогрессивно показывать контент, устранять waterfall-запросы и потоково отправлять HTML с сервера. В сочетании с `use()`, конкурентным рендерингом и Error Boundaries, Suspense является краеугольным камнем современной архитектуры React-приложений.
