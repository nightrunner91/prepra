...
1. [Когда нужен](#когда-нужен)
2. [Синтаксис](#синтаксис)
3. [Базовый пример](#базовый-пример)
4. [Зависимости](#зависимости)
5. [React 19: ref как проп](#react-19-ref-как-проп)
6. [Практический пример: VideoPlayer](#практический-пример-videoplayer)
7. [TypeScript](#typescript)
8. [Аналогия с Vue](#аналогия-с-vue)
9. [Когда использовать](#когда-использовать)
10. [Когда НЕ использовать](#когда-не-использовать)
...
- `useImperativeHandle` определяет публичный API компонента, доступный через `ref`.
- Работает в связке с `forwardRef` (до React 19) или с `ref` как пропом (React 19+).
- Третий аргумент — массив зависимостей, аналогичный `useEffect`.
- Типичные сценарии: фокус, прокрутка, управление плеером, анимации.
- Не используется для передачи данных — для этого есть props и callback-пропсы.
- Vue-аналог: `defineExpose()` — ограничивает публичный API компонента.
- В React 19 `forwardRef` не обязателен — `ref` передаётся как обычный проп.

## Заключение

`useImperativeHandle` — мощный, но редко нужный инструмент. Используйте его, когда без императивного доступа к DOM или внутренним методам не обойтись: фокус, прокрутка, управление плеером, интеграция с императивными библиотеками. Во всех остальных случаях отдавайте предпочтение props и композиции — это делает код более предсказуемым и тестируемым.
