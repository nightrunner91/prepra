...
questions:
  - "Что такое queryKey и зачем он должен быть стабильным?"
  - "В чём разница между staleTime и gcTime?"
  - "Как реализовать оптимистичные обновления с откатом при ошибке?"
  - "Когда использовать параметр enabled в useQuery?"
  - "Как инвалидировать кэш после успешной мутации?"
  - "Где хранится кэш TanStack Query по умолчанию и как сохранить его между перезагрузками?"
  - "Чем TanStack Query отличается от Zustand — когда что использовать?"
  - "Как реализовать бесконечную прокрутку с useInfiniteQuery?"
  - "Что происходит с кэшем при размонтировании компонента?"
---

# TanStack Query: кэш, мутации, серверное состояние

При работе с сервером каждый компонент обычно вручную управляет состояниями loading, error и success, дублируя бойлерплейт снова и снова. TanStack Query решает эту проблему: один хук useQuery заменяет useState + useEffect и добавляет кэширование, дедупликацию запросов, фоновое обновление и оптимистичные обновления. В статье разобраны ключевые концепции queryKey, staleTime, gcTime, паттерны мутаций и сравнение с Zustand.
...
**staleTime** определяет, сколько данные считаются актуальными. После истечения TanStack Query помечает данные как устаревшие (stale) и запускает **фоновое обновление** при следующем событии.

```
Компонент монтируется → Запрос к API → Данные получены
         ↓
    [staleTime: 5 минут]
    Данные считаются "свежими"
    НО новых запросов НЕ делается
         ↓
    Через 5 минут → Данные становятся "устаревшими" (stale)
         ↓
    Следующее событие (фокус окна, повторный рендер,
    подписка нового компонента) → Фоновый запрос к API
         ↓
    Новые данные получены → Таймер staleTime запускается ВНОВЬ
```

**Важно:** staleTime НЕ делает запрос автоматически по таймеру. Он только **помечает** данные как устаревшие. Запрос происходит при следующем событии:
- Фокус окна (`refetchOnWindowFocus: true` — по умолчанию)
- Переподключение сети (`refetchOnReconnect: true`)
- Монтирование нового компонента с тем же `queryKey`
- Вызов `refetch()` вручную

```jsx
// Пример: данные устарели, но запроса нет
// Пользователь видит старые данные
// Пока не переключит вкладку → фокус окна → фоновый запрос
```

#### Как работает gcTime

**gcTime** (ранее `cacheTime`) определяет, сколько кэш хранится в памяти **после того, как все компоненты отписались**.

**Что значит "все компоненты отписались":**

Компонент **подписан** на запрос, пока он смонтирован и использует `useQuery` с данным `queryKey`. Когда компонент размонтирован (ушёл со страницы, удалили из DOM), он **отписывается**.
...
function App() {
  ...
  return (
    <div>
      ...
      {showUsers && <UserList />}
      {/* UserList смонтирован → подписан на ["users"] */}
      {/* showUsers = false → UserList размонтирован → отписался */}
    </div>
  );
}
```
...
```
Компонент монтируется → Запрос → Данные в кэше
         ↓
    Компонент используется
    Кэш активен, данные доступны
         ↓
    Компонент размонтирован → Все подписчики отписались
         ↓
    [gcTime: 10 минут]
    Кэш всё ещё в памяти, но помечен на удаление
         ↓
    Если за 10 минут компонент снова смонтируется
    → Кэш используется (НЕ делается новый запрос)
         ↓
    Если за 10 минут компонент НЕ смонтировался
    → Кэш удаляется из памяти
         ↓
    При следующем монтировании → Новый запрос к API
```

#### Взаимодействие staleTime и gcTime
...
useQuery({
  ...
  gcTime: 10 * 60 * 1000,    // 10 минут жизни кэша
});
```

**Сценарий 1: Компонент постоянно смонтирован**

```
T=0:   Монтирование → Запрос → Данные получены
T=0-5: Данные свежие, запросов нет
T=5:   Данные устарели (stale)
T=5+:  При фокусе окна → фоновый запрос → таймер staleTime сбрасывается
       Кэш активен всё время
```

**Сценарий 2: Компонент размонтирован через 3 минуты**

```
T=0:   Монтирование → Запрос → Данные получены
T=0-3: Компонент активен, данные свежие
T=3:   Компонент размонтирован → Все отписались
       Запускается gcTime (10 минут)
T=3-13: Кэш в памяти, но не используется
T=5:   (Если бы компонент был активен) Данные устарели бы
       Но компонент размонтирован, поэтому ничего не происходит
T=13:  gcTime истёк → Кэш удалён из памяти
T=15:  Компонент снова смонтирован → Новый запрос к API
```

**Сценарий 3: Компонент размонтирован и смонтирован до gcTime**

```
T=0:   Монтирование → Запрос → Данные получены
T=3:   Компонент размонтирован → gcTime запущен (10 минут)
T=8:   Компонент снова смонтирован
       gcTime ещё не истёк → Кэш используется
       НО данные устарели (staleTime=5 минут истёк в T=5)
       → Фоновый запрос к API → Новые данные
       → staleTime сбрасывается
       → gcTime сбрасывается (пока компонент смонтирован)
```

#### Практические рекомендации

```jsx
// Короткоживущие данные (актуальны 30 секунд)
useQuery({
  queryKey: ["notifications"],
  queryFn: fetchNotifications,
  staleTime: 30 * 1000,        // 30 секунд свежести
  gcTime: 5 * 60 * 1000,       // 5 минут жизни кэша
});

// Средние данные (актуальны 5 минут)
useQuery({
  queryKey: ["users"],
  queryFn: fetchUsers,
  staleTime: 5 * 60 * 1000,    // 5 минут свежести
...
// Долгоживущие данные (актуальны час)
useQuery({
  queryKey: ["settings"],
  queryFn: fetchSettings,
  staleTime: 60 * 60 * 1000,   // 1 час свежести
  gcTime: 24 * 60 * 60 * 1000, // 24 часа жизни кэша
});
```

#### Итог

| Параметр | Что делает | Когда запускается | Что происходит по истечению |
|----------|-----------|-------------------|----------------------------|
| **staleTime** | Определяет свежесть данных | После получения данных | Данные помечаются устаревшими, фоновый запрос при следующем событии |
| **gcTime** | Определяет жизнь кэша в памяти | После отписки всех компонентов | Кэш удаляется из памяти |

**staleTime** — про актуальность данных.
**gcTime** — про память и производительность.
...
- TanStack Query управляет серверным состоянием: загрузка, кэширование, синхронизация с сервером, мутации.
- queryKey — уникальный идентификатор запроса; используется для кэширования, дедупликации и инвалидации.
- staleTime определяет, сколько данные считаются свежими; gcTime — сколько кэш живёт в памяти после отписки всех компонентов.
- Кэш TanStack Query хранится в RAM и теряется при перезагрузке страницы; persistQueryClient позволяет сохранить его в localStorage.
- useMutation используется для POST/PUT/DELETE; onMutate реализует оптимистичные обновления с откатом через onError.
- invalidateQueries помечает запросы устаревшими и запускает повторный fetch при следующем событии (фокус, реконнект, монтирование).
- useInfiniteQuery обеспечивает бесконечную прокрутку с курсором через getNextPageParam.
- enabled: !!id позволяет делать условные запросы, которые выполняются только при наличии зависимости.
- TanStack Query не заменяет Zustand: первый — для серверного состояния, второй — для UI-состояния.
- queryFn должна либо вернуть данные, либо выбросить ошибку — иначе TanStack Query не сможет обработать состояние ошибки.
