...
questions:
  ...
  - "Чем staleTime отличается от gcTime и как они определяют жизненный цикл данных в кэше"
  - "Как useMutation с onMutate реализует оптимистичные обновления и как onError + onSettled обеспечивают откат"
  - "Как invalidateQueries и enabled управляют актуальностью данных и условными запросами"
  - "Чем TanStack Query отличается от Zustand и как они дополняют друг друга в приложении"
---
...
- TanStack Query управляет серверным состоянием: загрузка, кэширование, дедупликация и синхронизация с сервером. `queryKey` — уникальный идентификатор запроса для кэширования, дедупликации и инвалидации.
- `staleTime` определяет, сколько данные считаются свежими; `gcTime` — сколько кэш живёт в памяти после отписки всех компонентов. Кэш хранится в RAM; `persistQueryClient` сохраняет его в localStorage между перезагрузками.
- `useMutation` для POST/PUT/DELETE: `onMutate` реализует оптимистичные обновления, `onError` + `onSettled` — откат при ошибке. `invalidateQueries` помечает запросы устаревшими и запускает повторный fetch при фокусе, реконнекте или монтировании.
- `useInfiniteQuery` обеспечивает бесконечную прокрутку с курсором через `getNextPageParam`; `enabled: !!id` — условные запросы, выполняющиеся только при наличии зависимости.
- `queryFn` должна вернуть данные или выбросить ошибку — иначе TanStack Query не обработает состояние ошибки корректно.
- TanStack Query и Zustand не заменяют друг друга: первый — для серверного состояния (кэш, мутации, пагинация), второй — для UI-состояния (модалки, корзина, аутентификация).
