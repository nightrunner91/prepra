...
questions:
  - "В чём разница между localStorage и sessionStorage по времени жизни и области видимости?"
  - "Почему нельзя хранить токены аутентификации в localStorage?"
  - "Что такое HttpOnly cookie и как он защищает от XSS?"
  - "Как атрибут SameSite защищает cookie от CSRF-атак?"
  - "Когда стоит использовать IndexedDB вместо localStorage?"
  - "Как синхронизировать состояние между вкладками через localStorage?"
  - "Что происходит при попытке записи в localStorage больше допустимого лимита?"
  - "Как правильно хранить объекты в localStorage?"
  - "Какие данные нельзя хранить в клиентских хранилищах?"
---
...
- localStorage хранит данные бессрочно и работает синхронно; sessionStorage — только до закрытия вкладки, изолирована по вкладкам.
- Web Storage API хранит только строки — объекты нужно сериализовать через JSON.stringify и десериализовать через JSON.parse.
- Cookies автоматически отправляются с каждым HTTP-запросом к домену; localStorage и sessionStorage — нет.
- HttpOnly-cookie недоступен из JavaScript — это основная защита токенов аутентификации от XSS-атак.
- Атрибут SameSite=Strict/Lax защищает cookie от CSRF; Secure — разрешает передачу только по HTTPS.
- IndexedDB — асинхронный NoSQL движок в браузере: поддерживает транзакции, индексы, бинарные данные и до нескольких гигабайт.
- Синхронный API localStorage блокирует главный поток; для больших данных используйте IndexedDB.
- При изменении localStorage в одной вкладке событие storage срабатывает во всех других вкладках того же домена.
- Для IndexedDB используют библиотеки idb или Dexie.js — нативный API избыточно многословен.
