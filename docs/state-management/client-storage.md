...
questions:
  - "Чем localStorage, sessionStorage и cookies различаются по объёму, времени жизни и области видимости данных"
  - "Почему токены аутентификации нельзя хранить в localStorage и как HttpOnly-cookie с SameSite защищают от XSS и CSRF"
  - "Когда стоит выбрать IndexedDB вместо localStorage и какие последствия у хранения больших объёмов данных в localStorage"
  - "Как работает синхронизация между вкладками через событие storage и какие сценарии она покрывает"
  - "Как правильно интегрировать клиентские хранилища с React и почему работу с ними инкапсулируют в хуки"
  - "Как комбинируются разные хранилища в одном приложении: cookies для токенов, localStorage для настроек, IndexedDB для офлайн-кэша"
---

# Клиентские хранилища: localStorage, cookies, IndexedDB

В браузере есть четыре механизма хранения данных: localStorage, sessionStorage, cookies и IndexedDB. Каждый имеет свои ограничения по объёму, времени жизни и уровню доступа. Неправильный выбор — например, хранение токена аутентификации в localStorage вместо HttpOnly-cookie — приводит к уязвимостям безопасности. Статья разбирает все четыре механизма, их отличия, интеграцию с React и типичные ошибки.
...
- localStorage хранит данные бессрочно и работает синхронно; sessionStorage — до закрытия вкладки, изолирована по вкладкам. Web Storage принимает только строки — объекты сериализуются через `JSON.stringify`/`JSON.parse`.
- Cookies автоматически отправляются с каждым HTTP-запросом к домену; `HttpOnly` делает cookie недоступным из JavaScript — основная защита токенов от XSS. `SameSite=Strict/Lax` защищает от CSRF, `Secure` — передача только по HTTPS.
- IndexedDB — асинхронный NoSQL движок: транзакции, индексы, бинарные данные, объём до нескольких гигабайт. Нативный API многословен, поэтому используют `idb` или `Dexie.js`.
- Синхронный API localStorage блокирует главный поток — для больших данных и частых операций используйте IndexedDB.
- При изменении localStorage в одной вкладке событие `storage` срабатывает во всех других вкладках того же домена — это используется для межвкладочной синхронизации.
- В React инкапсулируйте работу с хранилищами в кастомные хуки (`useLocalStorage`, `useCookie`) — не работайте с ними напрямую в компонентах.
