...
questions:
  ...
  - "Почему при деструктуризации стора Pinia теряется реактивность и как storeToRefs её сохраняет"
  - "Как $patch обеспечивает атомарные обновления и как плагины расширяют все сторы глобально"
  - "Как тестировать сторы Pinia изолированно и зачем нужен setActivePinia перед каждым тестом"
  - "Когда использовать Options API-стор, а когда Setup API-стор и какие у каждого ограничения"
  - "Как разделить ответственность между Pinia и TanStack Query / VueQuery в Vue-приложении"
---
...
- Pinia — официальная замена Vuex для Vue 3: нет mutations, actions могут быть синхронными и асинхронными. `defineStore` принимает Options API (объект) или Setup API (функция); `state` — функция для независимых экземпляров при SSR.
- Прямые мутации `this.field = value` — стандарт: Vue отслеживает изменения через Proxy автоматически. Getters — кэшиемые вычисляемые свойства, пересчитываются только при изменении зависимостей.
- `storeToRefs()` обязателен при деструктуризации стора — без него реактивные свойства превращаются в обычные примитивы и теряется связь с реактивной системой Vue.
- `$patch({ ... })` обновляет несколько полей атомарно; `$patch(fn)` — для сложных мутаций с массивами. Плагины через `pinia.use(...)` расширяют все сторы глобально (логирование, персистентность, аналитика).
- Для изолированного тестирования сторов — `setActivePinia(createPinia())` перед каждым тестом.
- Серверное состояние (данные с API) — зона TanStack Query / VueQuery (кэш, инвалидация, мутации); Pinia — для UI-состояния (модалки, корзина, аутентификация).
