...
questions:
  - "Как паттерн create(set, get) в Zustand устраняет необходимость в провайдерах и чем это отличается от Context или Redux"
  - "Почему в Zustand обязательны селекторы для подписки на часть состояния и как они предотвращают лишние ре-рендеры"
  - "Как set() обеспечивает иммутабельные обновления и как persist middleware с partialize сохраняет только нужные поля"
  - "Когда стоит разделять одно хранилище на несколько доменных и какие проблемы это решает"
  - "Как Immer middleware меняет правила обновления состояния и чем Zustand отличается от Redux Toolkit"
---

# Zustand: стор без бойлерплейта, селекторы, middleware

Управление глобальным состоянием в React традиционно требовало Redux с его провайдерами, редукторами и экшенами. Zustand предлагает альтернативу: один хук create, никаких провайдеров и полный доступ к состоянию из любой точки приложения. Библиотека весит около 1 КБ и хорошо сочетается с TanStack Query для серверных данных. В статье разобраны ключевые концепции, паттерны применения и миграция с Redux.
...
- Zustand создаёт хранилище через `create(set, get)` — никаких провайдеров и обёрток. Доступ к состоянию вне компонентов: `useStore.getState()`.
- Селекторы `useStore(state => state.field)` подписывают компонент только на нужную часть состояния и предотвращают лишние ре-рендеры.
- `set()` требует иммутабельности — нужно возвращать новый объект, не мутировать старый (в отличие от Pinia/Vue). `persist` middleware сохраняет состояние в localStorage; `partialize` позволяет сохранять только выбранные поля.
- Хранилища разделяют по доменам: `useAuthStore`, `useCartStore`, `useUIStore`. Immer middleware позволяет мутировать вложенные объекты через черновики, не нарушая иммутабельность.
- `devtools` middleware интегрируется с Redux DevTools для отладки.
- Zustand — для UI-состояния (модалки, корзина, аутентификация, настройки); серверные данные — зона TanStack Query.
