...
questions:
  - "Почему в Zustand нужны селекторы, а в Pinia — нет?"
  - "Как persist middleware сохраняет только часть состояния?"
  - "Как получить состояние Zustand вне React-компонента?"
  - "В чём разница между set() и прямой мутацией в Zustand?"
  - "Когда стоит разделять одно большое хранилище на несколько?"
  - "Как Immer middleware меняет правила обновления состояния?"
  - "Чем Zustand отличается от Redux Toolkit по бойлерплейту?"
  - "Что такое слайсы (slices) в Zustand и зачем они нужны?"
---
...
- Zustand создаёт хранилище через create(set, get) — никаких провайдеров, никаких обёрток.
- Селекторы useStore(state => state.field) позволяют компоненту подписываться только на нужную часть состояния и предотвращают лишние ре-рендеры.
- set() в Zustand требует иммутабельности — нужно возвращать новый объект, не мутировать старый (в отличие от Pinia/Vue).
- persist middleware сохраняет состояние в localStorage; partialize позволяет сохранять только выбранные поля.
- devtools middleware интегрируется с Redux DevTools для отладки.
- Доступ к состоянию вне компонентов: useStore.getState() — аналог $state в Pinia.
- Хранилища лучше разделять по доменам: useAuthStore, useCartStore, useUIStore.
- Immer middleware позволяет мутировать вложенные объекты через черновики, не нарушая иммутабельность.
- Zustand подходит для UI-состояния: модальные окна, корзина, аутентификация, настройки. Серверные данные — зона ответственности TanStack Query.
