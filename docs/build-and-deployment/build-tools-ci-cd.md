...
questions:
  - "Чем Vite отличается от Webpack по принципу работы в режиме разработки?"
  - "Что такое граф зависимостей и как бандлер его использует?"
  - "Почему tree shaking работает только с ES-модулями, а не с CommonJS?"
  - "Как React.lazy и Suspense реализуют code splitting по маршрутам?"
  - "В чём разница между CI и CD и зачем это фронтенд-разработчику?"
  - "Как работает кэширование зависимостей в GitHub Actions?"
  - "Что такое multi-stage build в Docker и зачем он нужен для фронтенда?"
  - "Что такое preview deployments и как они помогают в code review?"
---

# Webpack, Vite и CI/CD для фронтендера

Между написанием JSX и его запуском в браузере стоит целый конвейер: бандлер, транспилятор, оптимизатор, CI-сервер и платформа деплоя. Понимание каждого звена помогает не только настраивать проекты, но и объяснять, почему код работает в dev, но ломается в prod. Статья охватывает Webpack и Vite, code splitting и tree shaking, GitHub Actions, Docker и preview deployments.
...
- Бандлер строит граф зависимостей, трансформирует код (JSX, TypeScript → JS) и оптимизирует его для браузера.
- Webpack обрабатывает всё через лоадеры и плагины, но медленно стартует на больших проектах из-за обработки всего графа при запуске.
- Vite использует нативные ES-модули в dev-режиме — не создаёт бандл, а отдаёт файлы по требованию браузера.
- В production Vite использует Rollup, который создаёт оптимизированный бандл с хорошим tree shaking.
- Code splitting через `React.lazy` и `Suspense` позволяет загружать страницы и компоненты только когда они нужны.
- Tree shaking удаляет неиспользуемый код на основе статического анализа ES-импортов; CommonJS не поддерживается.
- CI проверяет код при каждом push (линт, тесты, сборка); CD автоматически деплоит после успешного CI.
- `npm ci` предпочтительнее `npm install` в CI: детерминированная установка строго по lock-файлу.
- Multi-stage Docker build уменьшает образ с ~500 МБ до ~20 МБ: production-этап содержит только nginx и статику.
- Preview deployments создают уникальный URL для каждого PR — ревьюеры тестируют изменения до мержа.
