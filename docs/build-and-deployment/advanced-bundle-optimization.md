...
questions:
  ...
  - "Почему tree shaking не работает с CommonJS-модулями?"
  - "Что означает `sideEffects: false` в package.json и как это влияет на удаление мёртвого кода?"
  - "Какие стратегии code splitting существуют: по маршрутам, компонентам, условиям?"
  - "Зачем разделять vendor chunks по категориям и как это связано с долгосрочным кэшированием?"
  - "Как `npm dedupe` и поле `overrides` устраняют дублирующиеся зависимости в бандле?"
  - "Чем Brotli лучше gzip и когда стоит использовать предварительное сжатие?"
  - "Как настроить бюджет бандла и автоматически проверять его в CI через bundlesize или size-limit?"
---

# Анализ и оптимизация JavaScript-бандла

Размер бандла определяет, насколько быстро пользователь увидит работающее приложение. Каждый лишний килобайт JavaScript увеличивает время разбора и выполнения, напрямую влияя на конверсию и Core Web Vitals. Статья разбирает инструменты анализа, принципы tree shaking и code splitting, стратегии работы с vendor chunks, борьбу с дубликатами и автоматический контроль через bundle budget.
...
- Размер бандла влияет на конверсию: каждые 100 КБ уменьшают её на 1–2%, превышение 3 секунд загрузки отталкивает 53% пользователей.
- webpack-bundle-analyzer и rollup-plugin-visualizer визуализируют состав бандла — ищите большие блоки, дубликаты и мёртвый код.
- Tree shaking работает только с ES-модулями: CommonJS, side effects и `export * from` блокируют удаление неиспользуемого кода.
- `sideEffects: false` в `package.json` разрешает бандлеру агрессивно удалять неиспользуемые модули.
- Code splitting по маршрутам — самая эффективная оптимизация: пользователь загружает только код нужной страницы.
- Разделение vendor chunks по категориям (React, UI, utils) улучшает кэширование: обновление одной библиотеки не инвалидирует весь vendor chunk.
- `contenthash` в именах файлов и директива `immutable` в Cache-Control обеспечивают долгосрочное кэширование статики.
- `npm dedupe` и `overrides` / `resolutions` устраняют дублирующиеся версии зависимостей в бандле.
- Brotli сжимает на 15–20% лучше gzip; предварительное сжатие при сборке снимает нагрузку с сервера.
- bundlesize и size-limit интегрируются в CI — превышение бюджета блокирует мерж PR.
