...
questions:
  - "Что такое секрет и почему клиентский бандл — не надёжное хранилище?"
  - "Как переменные `NEXT_PUBLIC_` и `VITE_` попадают в бандл?"
  - "Чем `runtimeConfig` в Nuxt отличается от `appConfig`?"
  - "Зачем нужен серверный прокси для внешних API с секретами?"
  - "Какие хранилища секретов существуют и в чём их отличия?"
  - "Как проводить zero-downtime ротацию секретов?"
  - "Как защитить `.env` от попадания в git?"
  - "Что делать, если секрет всё-таки попал в git?"
  - "Как pre-commit hooks и `gitleaks` помогают предотвратить утечки?"
---

# Управление секретами во фронтенде

Секреты — API-ключи, пароли к базам, приватные ключи JWT, `client_secret` OAuth — это точки входа в систему. Фундаментальное правило веба: всё, что попадает в JavaScript-бандл, может быть прочитано. Одна ошибка с префиксом `NEXT_PUBLIC_` — и секретный ключ Stripe оказывается в DevTools. Статья разбирает механику подстановки переменных в Next.js, Nuxt, Vite, архитектуру серверного прокси, хранилища секретов (Vault, AWS Secrets Manager, Doppler) и защиту git-репозитория от утечек.
...
- Всё, что попадает в JavaScript-бандл, доступно пользователю через DevTools — секреты нельзя хранить в клиентском коде.
- Префиксы `NEXT_PUBLIC_`, `VITE_`, `REACT_APP_`, `NUXT_PUBLIC_` встраиваются в бандл на этапе сборки; серверные переменные — без префикса.
- Утечки бывают не только через префикс: импорт серверного модуля в клиентский компонент, логи, source maps в production.
- В Nuxt `runtimeConfig` разделяет `runtimeConfig.public` (клиент) и корневые ключи (только сервер) — читается через `useRuntimeConfig()`.
- Правильная архитектура — серверный прокси: клиент → ваш сервер (Next.js API / Nuxt server route) → внешний API с секретом.
- Специализированные vaults (AWS Secrets Manager, HashiCorp Vault, Doppler, Infisical) дают централизованное управление, аудит доступа и автоматическую ротацию.
- Ротация — регулярная замена секретов; API-ключи — раз в 90 дней, JWT-секреты — при компрометации или раз в год.
- Zero-downtime ротация: генерируем новый секрет, поддерживаем оба, обновляем потребителей, отключаем старый.
- `.gitignore` для `.env`, pre-commit hooks (`gitleaks`, `truffleHog`), сканирование репозитория — три уровня защиты от утечек.
- Если секрет попал в git — сразу отзывайте, удаляйте из истории (`git filter-repo`, BFG), проверяйте логи на злоупотребления.

## Заключение

Управление секретами — это не только про `.env`. Это архитектурный вопрос: какие данные попадают в браузер, как они передаются, где хранятся и как часто меняются. Одна ошибка — `NEXT_PUBLIC_STRIPE_SECRET_KEY` вместо `STRIPE_SECRET_KEY` — может стоить тысячи долларов на неавторизованные платежи. Для SOC2 и CASA важно показать разделение секретов по окружениям, отсутствие секретов в клиентском коде, использование vaults и ротации, автоматическое сканирование на утечки. Фронтенд-разработчик должен уверенно объяснять, почему `NEXT_PUBLIC_` не может содержать секреты, и уметь спроектировать серверный прокси.
