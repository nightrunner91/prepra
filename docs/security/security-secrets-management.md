...
questions:
  ...
  - "Как префиксы `NEXT_PUBLIC_`, `VITE_` и `runtimeConfig.public` в Nuxt встраивают переменные в бандл и какие бывают скрытые утечки"
  - "Как серверный прокси скрывает секреты от клиента и почему это единственная правильная архитектура"
  - "Какие хранилища секретов (Vault, AWS Secrets Manager, Doppler) дают централизованное управление и автоматическую ротацию"
  - "Как проводить zero-downtime ротацию секретов и с какой частотой менять API-ключи и JWT-секреты"
  - "Какие три уровня защиты `.env` от попадания в git и что делать, если секрет всё-таки утекает"
  - "Как `gitleaks` и pre-commit hooks предотвращают утечки на этапе коммита"
---
...
- Всё, что попадает в JavaScript-бандл, доступно через DevTools; префиксы `NEXT_PUBLIC_`, `VITE_`, `REACT_APP_`, `NUXT_PUBLIC_` встраиваются в бандл на этапе сборки, серверные переменные — без префикса.
- Утечки бывают не только через префикс: импорт серверного модуля в клиентский компонент, логи, source maps в production; в Nuxt `runtimeConfig` разделяет `runtimeConfig.public` (клиент) и корневые ключи (только сервер).
- Правильная архитектура — серверный прокси: клиент → ваш сервер (Next.js API / Nuxt server route) → внешний API с секретом.
- Специализированные vaults (AWS Secrets Manager, HashiCorp Vault, Doppler) дают централизованное управление, аудит доступа и автоматическую ротацию; API-ключи меняют раз в 90 дней, JWT-секреты — при компрометации.
- Zero-downtime ротация: генерируем новый секрет, поддерживаем оба, обновляем потребителей, отключаем старый.
- `.gitignore` для `.env`, pre-commit hooks (`gitleaks`, `truffleHog`), сканирование репозитория — три уровня защиты; если секрет попал в git — сразу отзывайте, удаляйте из истории (`git filter-repo`, BFG).
