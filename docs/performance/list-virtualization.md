...
questions:
  - "Как виртуализация снижает количество DOM-узлов и почему padding имитирует полную высоту списка"
  - "Почему использование index как key ломает DOM при сортировке и фильтрации виртуализированного списка"
  - "Как выбрать между react-window и react-virtuoso в зависимости от требований к функциональности"
  - "Что такое overscanCount и как найти баланс между видимой подгрузкой и лишним рендером"
  - "При каком количестве элементов виртуализация оправдана и когда она становится over engineering"
---
...
- Виртуализация рендерит только видимые элементы (~50 DOM-узлов вместо тысяч). Контейнер имитирует полную высоту через `padding-top`/`padding-bottom`, DOM-узлы переиспользуются при скролле.
- Фиксированная высота (`FixedSizeList`) быстрее динамической — не нужно измерять каждый элемент. `react-window` минималистичен (~6 KB), `react-virtuoso` богаче функционально (sticky headers, infinite scroll, автоскролл, автоматический resize).
- `@tanstack/vue-virtual` — headless-решение: даёт только логику виртуализации, разметку пишешь сам. Подходит, когда нужен полный контроль над HTML.
- Оптимальный `overscanCount` — 5–10 элементов: меньше — видна «подгрузка» при быстром скролле, больше — лишний рендер невидимых узлов.
- Index как key ломает DOM при сортировке и фильтрации — React переиспользует узлы по позиции, а не по содержимому. Всегда используй стабильный ID.
