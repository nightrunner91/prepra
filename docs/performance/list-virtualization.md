...
questions:
  - "Что такое виртуализация списков и какую проблему она решает?"
  - "Как виртуализация имитирует полную высоту списка с помощью padding?"
  - "В чём разница между FixedSizeList и VariableSizeList в react-window?"
  - "Когда использовать react-window, а когда react-virtuoso?"
  - "Что такое overscanCount и какое значение оптимально?"
  - "Почему нельзя использовать index в качестве key при виртуализации?"
  - "Что такое headless-решение и зачем нужен @tanstack/vue-virtual?"
  - "При каком количестве элементов стоит применять виртуализацию?"
---

# Виртуализация списков: принципы и библиотеки

Рендер тысяч элементов сразу создаёт тысячи DOM-узлов: браузер тратит ресурсы на layout, paint и память каждого из них. Виртуализация решает это, отображая только видимую область — ~50 DOM-узлов вместо 10 000. Статья разбирает принцип работы и основные библиотеки для React и Vue.
...
- Виртуализация рендерит только видимые элементы (~50 DOM-узлов) вместо всего списка.
- Контейнер имитирует полную высоту через `padding-top`/`padding-bottom` — скроллбар ведёт себя корректно.
- DOM-узлы переиспользуются при скролле: меняется их содержимое и позиция, а не создаются новые.
- Фиксированная высота (`FixedSizeList`) быстрее динамической — не нужно измерять каждый элемент.
- `react-window` — минималистичен (~6 KB), `react-virtuoso` — богаче функционально (sticky headers, infinite scroll, автоскролл).
- `@tanstack/vue-virtual` — headless: даёт только логику, разметку пишешь сам.
- Оптимальный `overscanCount` — 5–10 элементов: меньше — видна «подгрузка», больше — лишний рендер.
- Index как key ломает DOM при сортировке и фильтрации — всегда используй стабильный ID.
- Виртуализация оправдана от ~500 элементов; для меньших списков overhead библиотеки превышает выгоду.
- Для responsive-контейнеров `react-window` требует `AutoSizer`; `react-virtuoso` обрабатывает resize автоматически.

## Заключение

Виртуализация списков — незаменимый инструмент при работе с тысячами элементов: она снижает количество DOM-узлов с 10 000 до ~50, устраняет тормоза при рендере и скролле. Для React выбор между `react-window` и `react-virtuoso` определяется функциональностью: нужны sticky headers, infinite scroll или автоскролл — берите `react-virtuoso`, критичен размер бандла — `react-window`. В Vue самый простой старт — `vue-virtual-scroller`, полный контроль над разметкой — `@tanstack/vue-virtual`. Главное правило: применяйте виртуализацию только там, где она действительно нужна — при 500+ элементах.
