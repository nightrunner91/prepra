...
questions:
  - "Почему JavaScript однопоточен и как это влияет на UI?"
  - "Чем Web Worker отличается от useTransition и requestIdleCallback?"
  - "Как передать данные в воркер и получить результат?"
  - "Почему данные копируются при передаче в воркер и как избежать копирования?"
  - "Что такое transferable objects и когда их использовать?"
  - "В чём разница между Web Worker, Shared Worker и Service Worker?"
  - "Что такое inline worker и когда он удобен?"
  - "Как comlink упрощает работу с Web Workers?"
  - "При каком пороге вычислений стоит использовать Web Workers?"
---

# Web Workers: фоновые вычисления без блокировки UI

JavaScript в браузере работает в одном потоке — любое тяжёлое вычисление блокирует UI. Web Workers решают это, выполняя код в отдельном фоновом потоке. В отличие от `useTransition` и `requestIdleCallback`, они физически выносят задачу с Main Thread.

## Содержание

1. [Почему это важно: однопоточность JavaScript](#почему-это-важно-однопоточность-javascript)
2. [Базовый синтаксис Web Worker](#базовый-синтаксис-web-worker)
3. [Интеграция с React](#интеграция-с-react)
4. [Inline Workers: воркеры без отдельного файла](#inline-workers-воркеры-без-отдельного-файла)
5. [Shared Workers: один воркер для нескольких вкладок](#shared-workers-один-воркер-для-нескольких-вкладок)
6. [Service Workers: воркеры для сетевых запросов](#service-workers-воркеры-для-сетевых-запросов)
7. [Ограничения и подводные камни](#ограничения-и-подводные-камни)
8. [Когда использовать Web Workers](#когда-использовать-web-workers)
9. [Инструменты и библиотеки](#инструменты-и-библиотеки)
10. [Отладка Web Workers](#отладка-web-workers)
11. [Сравнение: Web Workers vs useTransition vs requestIdleCallback](#сравнение-web-workers-vs-usetransition-vs-requestidlecallback)
12. [Когда Web Workers действительно нужны](#когда-web-workers-действительно-нужны-а-когда--over-engineering)
...
- Web Workers выполняют JS в отдельном потоке, не блокируя UI даже при многосекундных вычислениях.
- Воркер не имеет доступа к DOM, `window` и React — общение только через `postMessage`.
- Данные при передаче копируются (structured clone); большие бинарные данные передаются без копирования через transferable objects (ArrayBuffer).
- `useTransition` и `useDeferredValue` не освобождают Main Thread — они лишь откладывают обновления; воркер — единственный способ физически вынести вычисления.
- Inline workers создаются из Blob — удобно для небольших воркеров без отдельного файла.
- Shared Worker: один экземпляр на несколько вкладок одного origin; подходит для общего WebSocket или кэша.
- Service Worker: перехватывает сетевые запросы, работает даже без открытой вкладки; основа PWA.
- comlink (Google) скрывает `postMessage` за обычным API — вызываешь методы воркера как обычные функции через async/await.
- Порог применения: вычисление > 100мс или данные > 10 MB — используй воркер; CRUD, формы, списки до 1000 строк — не нужен.
- Оптимальный паттерн: Web Worker физически выносит вычисление + `useTransition` помечает обновление state как несрочное.

## Заключение

Web Workers — мощный, но нишевый инструмент. В 99% фронтенд-приложений хватает `useMemo`, `useTransition` и виртуализации. Воркеры нужны там, где вычисление реально блокирует Main Thread дольше 100 мс: графические редакторы, IDE, трейдинговые платформы, обработка видео. При их использовании общайтесь с воркером только через `postMessage`, передавайте большие данные как transferable objects и комбинируйте с `useTransition` для максимальной отзывчивости UI.
