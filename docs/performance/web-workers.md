...
questions:
  - "Почему однопоточность JavaScript блокирует UI и как Web Workers решают эту проблему на уровне потоков"
  - "Чем Web Worker фундаментально отличается от `useTransition` и `requestIdleCallback` в работе с Main Thread"
  - "Как transferable objects устраняют копирование данных при передаче между потоками и когда их применять"
  - "Чем Web Worker, Shared Worker и Service Worker различаются по области видимости и сценариям использования"
  - "При каком пороге вычислений Web Workers оправданы и как comlink упрощает работу с ними"
---
...
- Web Workers выполняют JS в отдельном потоке, не блокируя UI. Воркер не имеет доступа к DOM, `window` и React — общение только через `postMessage` (данные копируются через structured clone).
- Transferable objects (`ArrayBuffer`, `MessageChannel`) передаются без копирования — владение переходит к получателю. Критично для больших бинарных данных (> 10 MB).
- `useTransition` и `useDeferredValue` не освобождают Main Thread — они лишь откладывают обновления. Воркер — единственный способ физически вынести вычисления в отдельный поток.
- Shared Worker: один экземпляр на несколько вкладок одного origin (общий WebSocket, кэш). Service Worker: перехватывает сетевые запросы, работает без открытой вкладки — основа PWA.
- comlink (Google) скрывает `postMessage` за обычным API — вызываешь методы воркера как обычные функции через async/await, что убирает boilerplate.
- Порог применения: вычисление > 100 мс или данные > 10 MB. CRUD, формы, списки до 1000 строк — воркер не нужен. Оптимальный паттерн: воркер физически выносит вычисление + `useTransition` помечает обновление state как несрочное.
