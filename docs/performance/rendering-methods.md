...
questions:
  ...
  - "Как ISR и stale-while-revalidate позволяют обновлять статические страницы без полной пересборки"
  - "Почему React Server Components сокращают JS-бандл и какие ограничения они накладывают"
  - "Как Streaming SSR и Partial Prerendering используют Suspense для приоритизации загрузки"
  - "Какой метод рендеринга выбрать для блога, e-commerce и дашборда и почему"
---
...
- CSR рендерит страницу в браузере (плохо для SEO, быстрая навигация после загрузки); SSR генерирует HTML на каждый запрос (SEO-friendly, нагрузка на сервер); SSG генерирует HTML при сборке (максимальная производительность, данные устаревают).
- ISR = SSG + периодическое обновление через stale-while-revalidate: первый пользователь после истечения кэша видит старую версию, пока страница регенерируется. `revalidatePath` инвалидирует конкретный URL, `revalidateTag` — все данные с тегом.
- RSC выполняются только на сервере и не попадают в JS-бандл клиента — хуки и браузерные API недоступны. В App Router ~70-80% компонентов серверные, клиентские нужны только для интерактивности.
- Streaming SSR отправляет HTML чанками через Suspense, улучшая TTFB на страницах с тяжёлыми компонентами. Static Export — всё приложение статические файлы без Node.js; SSR, ISR и API Routes недоступны.
- PPR объединяет подходы: статическая оболочка генерируется при сборке, динамические части рендерятся через Suspense при запросе — один файл содержит и мгновенно отдаваемый HTML, и живой контент.
- Для большинства проектов оптимален гибридный подход: SSG/ISR для публичных страниц, RSC + Client Components для интерактивных разделов.
